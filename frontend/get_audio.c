...
#define GA_URS_IFLOOP( ga_urs_bps ) \
    ...
    if (swap_order == 0) {
        GA_URS_IFLOOP(1)
            * --op = (int) ((uint32_t) ip[i] << (b - 8));
        GA_URS_IFLOOP(2)
            * --op = (int) (((uint32_t) ip[i] << (b - 16)) | ((uint32_t) ip[i + 1] << (b - 8)));
        GA_URS_IFLOOP(3)
            * --op = (int) (((uint32_t) ip[i] << (b - 24)) | ((uint32_t) ip[i + 1] << (b - 16))
                            | ((uint32_t) ip[i + 2] << (b - 8)));
        GA_URS_IFLOOP(4)
            * --op = (int) (((uint32_t) ip[i] << (b - 32)) | ((uint32_t) ip[i + 1] << (b - 24))
                            | ((uint32_t) ip[i + 2] << (b - 16))
                            | ((uint32_t) ip[i + 3] << (b - 8)));
    }
    else {
        GA_URS_IFLOOP(1)
            * --op = (int) ((((uint32_t) (ip[i] ^ 0x80)) << (b - 8))
                            | ((uint32_t) 0x7f << (b - 16))); /* convert from unsigned */
        GA_URS_IFLOOP(2)
            * --op = (int) (((uint32_t) ip[i] << (b - 8)) | ((uint32_t) ip[i + 1] << (b - 16)));
        GA_URS_IFLOOP(3)
            * --op = (int) (((uint32_t) ip[i] << (b - 8)) | ((uint32_t) ip[i + 1] << (b - 16))
                            | ((uint32_t) ip[i + 2] << (b - 24)));
        GA_URS_IFLOOP(4)
            * --op = (int) (((uint32_t) ip[i] << (b - 8)) | ((uint32_t) ip[i + 1] << (b - 16))
                            | ((uint32_t) ip[i + 2] << (b - 24))
                            | ((uint32_t) ip[i + 3] << (b - 32)));
    }
#undef GA_URS_IFLOOP
...
parse_wave_header(lame_global_flags * gfp, FILE * sf)
{
    ...
    for (loop_sanity = 0; loop_sanity < 20; ++loop_sanity) {
        ...
        if (ui32_ckID == WAV_ID_FMT) {
            ...
            (void) ui32_nAvgBytesPerSec;
            (void) ui16_nBlockAlign;
            ui32_cksize -= 16u;
            ...
            if ((ui32_cksize > 9u) && (ui16_wFormatTag == WAVE_FORMAT_EXTENSIBLE)) {
                ...
                (void) ui16_cbSize;
                (void) ui16_wValidBitsPerSample;
                (void) ui32_dwChannelMask;
            }
