...
static int open_wav(char const *path, wav_input *input) {
  ...
  if (!input->file || !read_bytes(input->file, header, sizeof(header)) ||
  ...
  while (read_bytes(input->file, chunk, sizeof(chunk))) {
    ...
    if (memcmp(chunk, "fmt ", 4) == 0) {
      if (size < sizeof(format) ||
          !read_bytes(input->file, format, sizeof(format)))
        break;
      if (read_le16(format) != 1 || read_le16(format + 2) < 1 ||
          read_le16(format + 2) > 2) {
        break;
      }
      input->channels = read_le16(format + 2);
      input->sample_rate = read_le32(format + 4);
      input->bits_per_sample = read_le16(format + 14);
      if (input->bits_per_sample != 8 && input->bits_per_sample != 16 &&
          input->bits_per_sample != 24 && input->bits_per_sample != 32) {
        break;
      }
      have_format = 1;
      ...
      if (have_format)
        return input->data_bytes > 0;
      break;
    }

    if (fseek(input->file, payload + (long)((size + 1u) & ~1u), SEEK_SET) != 0)
      break;
  }
...
static int write_encoded(FILE *output, lame_t encoder, short *left,
                         short *right, int frames, unsigned char *buffer) {
  int bytes = lame_encode_buffer(encoder, left, right, frames, buffer,
                                 OUTPUT_BUFFER_SIZE);
  if (bytes < 0 ||
      (bytes > 0 && fwrite(buffer, 1, (size_t)bytes, output) != (size_t)bytes))
    return 0;
  return 1;
}

static size_t mp3_frame_size(unsigned char const *header, size_t remaining) {
  static int const mpeg1_bitrates[] = {32,  40,  48,  56,  64,  80,  96,
                                       112, 128, 160, 192, 224, 256, 320};
  static int const mpeg2_bitrates[] = {8,  16, 24, 32,  40,  48,  56,
...
static int validate_mp3_file(char const *path) {
  ...
  if (size >= 10 && memcmp(bytes, "ID3", 3) == 0) {
    size_t tag_size =
        ((size_t)(bytes[6] & 0x7f) << 21) | ((size_t)(bytes[7] & 0x7f) << 14) |
        ((size_t)(bytes[8] & 0x7f) << 7) | (size_t)(bytes[9] & 0x7f);
...
int main(int argc, char **argv) {
  ...
  if (!encoder || lame_set_num_channels(encoder, (int)input.channels) < 0 ||
      lame_set_in_samplerate(encoder, (int)input.sample_rate) < 0 ||
      lame_set_bWriteVbrTag(encoder, 0) < 0 ||
      (vbr ? lame_set_VBR(encoder, vbr_default) < 0 ||
                 lame_set_VBR_quality(encoder, 4.0f) < 0
  ...
  while (remaining > 0) {
    ...
    for (unsigned int frame = 0; frame < frames; ++frame) {
      ...
      right[frame] = input.channels == 2
                         ? sample_to_short(sample + bytes_per_sample,
...

  {
    ...
    if (bytes < 0 || (bytes > 0 && fwrite(encoded, 1, (size_t)bytes, output) !=
                                       (size_t)bytes)) {
