...
static int
read_bytes(FILE *file, unsigned char *buffer, size_t size)
{
    return fread(buffer, 1, size, file) == size;
}

static uint16_t
read_le16(unsigned char const *bytes)
{
    return (uint16_t)bytes[0] | ((uint16_t)bytes[1] << 8);
}

static uint32_t
read_le32(unsigned char const *bytes)
{
...
static int
open_wav(char const *path, wav_input *input)
{
    ...
    while (read_bytes(input->file, chunk, sizeof(chunk))) {
        ...
        if (memcmp(chunk, "fmt ", 4) == 0) {
            ...
            if (input->bits_per_sample != 8
                ...
                && input->bits_per_sample != 24
                && input->bits_per_sample != 32) {
                break;
            }
            have_format = 1;
...
static int16_t
sample_to_short(unsigned char const *sample, unsigned int bits)
{
    int32_t value;
...
static int
write_encoded(FILE *output, lame_t encoder, short *left, short *right, int frames,
...
static size_t
mp3_frame_size(unsigned char const *header, size_t remaining)
{
    static int const mpeg1_bitrates[] = {
        32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320
    };
    static int const mpeg2_bitrates[] = {
...
static int
validate_mp3_file(char const *path)
{
    FILE *file;
    unsigned char *bytes;
    long file_size;
    ...
    if (size >= 10 && memcmp(bytes, "ID3", 3) == 0) {
        size_t tag_size = ((size_t)(bytes[6] & 0x7f) << 21)
...
main(int argc, char **argv)
{
    ...
    if (!encoder
        || lame_set_num_channels(encoder, (int)input.channels) < 0
    ...
    while (remaining > 0) {
        ...
        if (frames > INPUT_FRAMES)
            frames = INPUT_FRAMES;
        if (frames == 0
            || fread(raw, frame_bytes, frames, input.file) != frames) {
        ...
        for (unsigned int frame = 0; frame < frames; ++frame) {
            ...
            right[frame] = input.channels == 2
                         ? sample_to_short(sample + bytes_per_sample, input.bits_per_sample)
                         : left[frame];
        }
