 ...
 *  coding by Glen Sawyer (mp3gain@hotmail.com) 735 W 255 N, Orem, UT 84057-4505
 * USA
...
#define RMS_PERCENTILE                                                         \
  0.95 /* percentile which is louder than the proposed level */
#define MAX_SAMP_FREQ 48000L /* maximum allowed sample frequency [Hz] */
#define RMS_WINDOW_TIME_NUMERATOR 1L
#define RMS_WINDOW_TIME_DENOMINATOR                                            \
  20L                    /* numerator / denominator = time slice size [s] */
#define STEPS_per_dB 100 /* Table entries per dB */
#define MAX_dB                                                                 \
  120 /* Table entries for 0...MAX_dB (normal max. values are 70...80 dB) */
...
enum {
  ...
  MAX_SAMPLES_PER_WINDOW = ((MAX_SAMP_FREQ * RMS_WINDOW_TIME_NUMERATOR) /
...
struct replaygain_data {
  ...
  Float_t *rout;
  long sampleWindow; /* number of samples required to reach number of
                        milliseconds required for RMS window */
  long totsamp;
  double lsum;
  double rsum;
  int freqindex;
