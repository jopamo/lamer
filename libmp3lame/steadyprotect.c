...
static steady_tonal_profile_t const steady_tonal_profiles[] = {{-0.75f, 0.84139514165f}, {-1.50f, 0.70794578438f}};

int steady_tonal_protect_mode(void) {
    return LAME_STEADY_TONAL_PROTECT_MODE;
}

static int steady_tonal_debug_requested(void) {
    char const* const env = getenv("LAME_STEADY_DEBUG");

    return env != 0 && *env != '\0';
}

void steady_tonal_stats_init_if_requested(lame_internal_flags* gfc) {
...
void steady_tonal_stats_free(lame_internal_flags* gfc) {
...
static FLOAT steady_tonal_sfb_center_hz(lame_internal_flags const* gfc, int sfb) {
...
static void steady_tonal_band_metric(lamer_dsp const* dsp, gr_info const* cod_info, int sfb, int start, FLOAT distort_sfb, FLOAT* noise_db, FLOAT* loss_db, FLOAT* failure_db) {
    int const width = cod_info->width[sfb];
    int const* const scalefac = cod_info->scalefac;
    int const* const ix = cod_info->l3_enc;
    int const scalefac_value = scalefac[sfb] + (cod_info->preflag ? pretab[sfb] : 0);
    FLOAT const step = POW20(cod_info->global_gain - scalefac_value * (1 << (cod_info->scalefac_scale + 1)) - cod_info->subblock_gain[cod_info->window[sfb]] * 8);
    FLOAT source_energy = 0.0f;
    FLOAT quant_energy = 0.0f;

    dsp->reconstructed_energy_f32(cod_info->xr + start, ix + start, step, width, &source_energy, &quant_energy);
...
int safe_steady_tonal_protect_allowed(lame_internal_flags const* gfc) {
...
static void steady_tonal_note_mask(unsigned long long by_sfb[SBMAX_l], unsigned int sfb_mask) {
    int sfb;
...
int steady_tonal_candidate_select(lame_internal_flags const* gfc, gr_info const* cod_info, int gr, int psy_ch, FLOAT const* distort, steady_tonal_candidate_t* candidate, char const** reject_reason) {
    ...
    *reject_reason = "eligible";
    ...
    if (psv->short_mask_final_mask[gr][psy_ch] != 0 || psv->short_mask_score_rel[gr][psy_ch] >= STEADY_TONAL_TRANSIENT_SCORE_CEIL) {
    ...
    for (sfb = 0; sfb < cod_info->psy_lmax && sfb < SBMAX_l; ++sfb) {
        ...
        steady_tonal_band_metric(&gfc->dsp, cod_info, sfb, j, distort[sfb], &noise_db, &loss_db, &failure_db);
        ...
        if (noise_db < STEADY_TONAL_NOISE_DB_FLOOR && loss_db <= STEADY_TONAL_LOSS_OK_DB) {
            j += cod_info->width[sfb];
            continue;
        }
...
void steady_tonal_build_xmin(FLOAT* dst, FLOAT const* src, gr_info const* cod_info, steady_tonal_candidate_t const* candidate, steady_tonal_profile_t const* profile, int* tightened_bands) {
    int sfb;
...
void steady_tonal_selected_failure(lame_internal_flags const* gfc, gr_info const* cod_info, FLOAT const* distort, unsigned int sfb_mask, steady_tonal_failure_t* failure) {
    int sfb, j = 0;
...
void steady_tonal_stats_note_frame(steady_tonal_stats_t* stats) {
    stats->frames_seen++;
}

void steady_tonal_stats_note_granule(steady_tonal_stats_t* stats, gr_info const* cod_info) {
    stats->granules_seen++;
    if (cod_info->block_type != SHORT_TYPE) {
...
void steady_tonal_stats_note_candidate(steady_tonal_stats_t* stats, steady_tonal_candidate_t const* candidate, steady_tonal_failure_t const* before) {
    stats->candidate_granules++;
    stats->selected_bands_total += candidate->sfb_count;
    stats->failure_before_sum += before->failure_db_sum;
    stats->max_failure_before = Max(stats->max_failure_before, (double)before->max_failure_db);
    steady_tonal_note_mask(stats->candidate_by_sfb, candidate->sfb_mask);
}

void steady_tonal_stats_note_retry(steady_tonal_stats_t* stats) {
    stats->retry_granules++;
}

void steady_tonal_stats_note_reject(steady_tonal_stats_t* stats) {
    stats->reject_granules++;
}

void steady_tonal_stats_note_accept(steady_tonal_stats_t* stats,
                                    int frame,
    ...
    stats->max_failure_after = Max(stats->max_failure_after, (double)after->max_failure_db);
    steady_tonal_note_mask(stats->accept_by_sfb, candidate->sfb_mask);

    if (stats->commit_event_count < STEADY_TONAL_COMMIT_EVENT_CAP) {
        steady_tonal_commit_event_t* const ev = &stats->commit_events[stats->commit_event_count++];
        ev->frame = frame;
        ev->gr = gr;
        ev->ch = ch;
...
void steady_tonal_dump_stats_if_requested(lame_internal_flags const* gfc) {
    ...
    fprintf(stderr,
            "steady_tonal_stats mode=%d cfg_vbr=%d cfg_short_blocks=%d "
            "cfg_free_format=%d "
            ...
            steady_tonal_protect_mode(), gfc->cfg.vbr, gfc->cfg.short_blocks, gfc->cfg.free_format, gfc->cfg.ATHonly, gfc->cfg.noATH, gfc->cfg.samplerate_out, safe_steady_tonal_protect_allowed(gfc),
            stats->frames_seen, stats->granules_seen, stats->long_granules_seen, stats->candidate_granules, stats->retry_granules, stats->accept_granules, stats->reject_granules,
            stats->selected_bands_total, stats->accepted_selected_bands_total, stats->failure_before_sum, stats->failure_after_sum, stats->max_failure_before, stats->max_failure_after,
    ...
    for (i = 0; i < stats->commit_event_count; ++i) {
        ...
        fprintf(stderr,
                ...
                i, ev->frame, ev->gr, ev->ch, ev->block_type, ev->selected_bands, ev->best_bits, (double)ev->failure_before_sum, (double)ev->failure_after_sum, (double)ev->max_before,
                (double)ev->max_after, ev->selected_sfb_mask_lo, ev->selected_sfb_mask_hi);
    }
}

int steady_tonal_profile_count(void) {
    return (int)dimension_of(steady_tonal_profiles);
}

steady_tonal_profile_t const* steady_tonal_profile_get(int profile_index) {
