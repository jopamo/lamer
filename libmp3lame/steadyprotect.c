...
static steady_tonal_profile_t const steady_tonal_profiles[] = {
    {-0.75f, 0.84139514165f}, {-1.50f, 0.70794578438f}};

int steady_tonal_protect_mode(void) { return LAME_STEADY_TONAL_PROTECT_MODE; }
...
static FLOAT steady_tonal_sfb_center_hz(lame_internal_flags const *gfc,
                                        int sfb) {
...
static void steady_tonal_band_metric(lamer_dsp const *dsp,
                                     gr_info const *cod_info, int sfb,
                                     int start, FLOAT distort_sfb,
                                     FLOAT *noise_db, FLOAT *loss_db,
                                     FLOAT *failure_db) {
  ...
  *loss_db = 0.0f;
  if (source_energy > 1E-20f && quant_energy < source_energy) {
    *loss_db =
        FAST_LOG10(Max(source_energy / Max(quant_energy, 1E-20f), 1E-20f));
    if (*loss_db < 0.0f) {
      *loss_db = 0.0f;
    }
  }
...
static void steady_tonal_note_mask(unsigned long long by_sfb[SBMAX_l],
...
int steady_tonal_candidate_select(lame_internal_flags const *gfc,
                                  gr_info const *cod_info, int gr, int psy_ch,
                                  FLOAT const *distort,
  ...
  *reject_reason = "eligible";
  ...
  if (psv->short_mask_final_mask[gr][psy_ch] != 0 ||
      psv->short_mask_score_rel[gr][psy_ch] >=
          STEADY_TONAL_TRANSIENT_SCORE_CEIL) {
  ...
  for (sfb = 0; sfb < cod_info->psy_lmax && sfb < SBMAX_l; ++sfb) {
    ...
    FLOAT const ath_margin_db =
        FAST_LOG10(Max(energy / Max(ath, 1E-20f), 1E-20f));
    FLOAT noise_db = 0.0f;
    FLOAT loss_db = 0.0f;
...
void steady_tonal_build_xmin(FLOAT *dst, FLOAT const *src,
...
void steady_tonal_selected_failure(lame_internal_flags const *gfc,
                                   gr_info const *cod_info,
                                   FLOAT const *distort, unsigned int sfb_mask,
...
void steady_tonal_stats_note_granule(steady_tonal_stats_t *stats,
                                     gr_info const *cod_info) {
  stats->granules_seen++;
  if (cod_info->block_type != SHORT_TYPE) {
...
void steady_tonal_stats_note_candidate(
    steady_tonal_stats_t *stats, steady_tonal_candidate_t const *candidate,
  ...
  stats->max_failure_before =
      Max(stats->max_failure_before, (double)before->max_failure_db);
...
void steady_tonal_stats_note_accept(steady_tonal_stats_t *stats, int frame,
                                    gr_info const *cod_info, int gr, int ch,
                                    int best_bits,
                                    steady_tonal_candidate_t const *candidate,
                                    steady_tonal_failure_t const *before,
  ...
  stats->max_failure_after =
      Max(stats->max_failure_after, (double)after->max_failure_db);
...
void steady_tonal_dump_stats_if_requested(lame_internal_flags const *gfc) {
  ...
  fprintf(
      ...
      steady_tonal_protect_mode(), gfc->cfg.vbr, gfc->cfg.short_blocks,
      gfc->cfg.free_format, gfc->cfg.ATHonly, gfc->cfg.noATH,
      gfc->cfg.samplerate_out, safe_steady_tonal_protect_allowed(gfc),
      stats->frames_seen, stats->granules_seen, stats->long_granules_seen,
      stats->candidate_granules, stats->retry_granules, stats->accept_granules,
      stats->reject_granules, stats->selected_bands_total,
      stats->accepted_selected_bands_total, stats->failure_before_sum,
      stats->failure_after_sum, stats->max_failure_before,
      stats->max_failure_after, stats->commit_event_count,
      stats->commit_event_overflow);
  ...
  for (i = 0; i < stats->commit_event_count; ++i) {
    ...
    fprintf(
        ...
        i, ev->frame, ev->gr, ev->ch, ev->block_type, ev->selected_bands,
        ev->best_bits, (double)ev->failure_before_sum,
        (double)ev->failure_after_sum, (double)ev->max_before,
        (double)ev->max_after, ev->selected_sfb_mask_lo,
        ev->selected_sfb_mask_hi);
  }
}
...
steady_tonal_profile_t const *steady_tonal_profile_get(int profile_index) {
  if (profile_index < 0 ||
      profile_index >= (int)dimension_of(steady_tonal_profiles)) {
    return 0;
  }
