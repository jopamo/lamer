...
typedef uint8_t (*find_sf_f)(const algo_t *, const FLOAT *, const FLOAT *,
                             FLOAT, unsigned int, uint8_t);
...
#define SHORT_EXPERIMENTAL_REDIST_PROFILE_MASK                                 \
  ((1u << 0) | (1u << 1) | (1u << 2) | (1u << 3))
#endif
...
static short_redist_profile_t const short_redist_profiles[] = {
    {10, -4.0f, 0.39810717055f, 12, 1.25f, 1.33352143216f, 11, 2.0f,
     1.58489319246f},
    {10, -6.0f, 0.25118864315f, 12, 0.75f, 1.18850222744f, 11, 1.25f,
     1.33352143216f},
    {10, -5.0f, 0.31622776602f, 12, 0.0f, 1.0f, 12, 0.75f, 1.18850222744f},
    {9, -5.0f, 0.31622776602f, 10, 0.5f, 1.12201845430f, 10, 4.0f,
     2.51188643151f}};
...
static void cutDistribution(const int sfwork[SFBMAX], int sf_out[SFBMAX],
                            int cut);
...
static void short_transient_window_noise(gr_info const *cod_info,
                                         FLOAT const *distort, int attack_win,
                                         short_window_noise_t *res) {
  int sfb;
...
static void short_transient_build_redistributed_xmin(
    FLOAT *redis_xmin, FLOAT const *orig_xmin, gr_info const *cod_info,
    int attack_win, short_redist_profile_t const *profile, int *tightened_bands,
    int *relaxed_bands) {
  int sfb;
...
static int short_redist_profile_better(
    short_window_noise_t const *candidate_attack,
    calc_noise_result const *candidate_global, int candidate_bits,
    short_window_noise_t const *best_attack,
    calc_noise_result const *best_global, int best_bits) {
...
static int steady_tonal_profile_better(
    steady_tonal_failure_t const *candidate,
    calc_noise_result const *candidate_global, int candidate_bits,
    steady_tonal_failure_t const *best, calc_noise_result const *best_global,
    int best_bits) {
...
static int short_redist_profile_enabled(short_redist_mode_t const *mode,
                                        int profile_index) {
...
static void short_transient_redistribute_retry(
    lame_internal_flags *gfc, algo_t that_[2][2],
    const FLOAT l3_xmin[2][2][SFBMAX], int sfwork_[2][2][SFBMAX],
    int vbrsfmin_[2][2][SFBMAX], int max_nbits_ch[2][2], int ngr, int nch,
    int use_nbits_ch[2][2], int use_nbits_gr[2], int *use_nbits_fr,
    int max_nbits_fr, short_redist_mode_t const *mode) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  ...
  for (gr2 = 0; gr2 < ngr; ++gr2) {
    for (ch2 = 0; ch2 < nch; ++ch2) {
      ...
      short_transient_window_noise(cod_info, old_distort, attack_win,
                                   &old_attack_noise);
      ...
      if (cfg->analysis) {
        fprintf(
            ...
            "old_over_count=%d\n",
            mode->trace_prefix, candidate, gfc->ov_enc.frame_number, gr2, ch2,
            psy_ch, short_mask_relax_source_name(psy_ch), mode->trace_prefix,
            candidate_reason, attack_win, (double)score_rel, final_mask,
            (double)impulse_ratio, (double)tail_ratio,
            mode->attack_profile_name, mode->trace_prefix, profiles_available,
            old_bits, (double)old_attack_noise.max_noise,
            (double)old_attack_noise.over_noise, old_attack_noise.over_count,
...

      {
        ...
        for (profile_index = 0;
          ...
          *cod_info = saved_gi;
          ...
          short_transient_build_redistributed_xmin(
              redis_xmin, l3_xmin[gr2][ch2], cod_info, attack_win, profile,
          ...
          calc_noise(cod_info, l3_xmin[gr2][ch2], new_distort,
                     &noise_retry_orig, 0);
          short_transient_window_noise(cod_info, new_distort, attack_win,
                                       &new_attack_noise);
          ...
          if (legal_profile &&
              (!have_legal_profile ||
               short_redist_profile_better(&new_attack_noise, &noise_retry_orig,
                                           new_bits, &best_attack_noise,
                                           &best_noise_orig, best_bits))) {
          ...
          *cod_info = saved_gi;
          ...
          trial_rollback_ok =
              ...
              fabsf(noise_restore_orig.max_noise - noise_orig.max_noise) <
                  1e-6f &&
              fabsf(noise_restore_orig.over_noise - noise_orig.over_noise) <
                  1e-6f &&
          ...
          if (cfg->analysis) {
            fprintf(
                ...
                mode->trace_prefix, gfc->ov_enc.frame_number, gr2, ch2, psy_ch,
                short_mask_relax_source_name(psy_ch), mode->trace_prefix,
                profile_index, mode->trace_prefix, legal_profile, attack_win,
                old_bits, new_bits, (double)impulse_ratio, (double)tail_ratio,
                ...
                old_attack_noise.over_count, new_attack_noise.over_count,
                (double)noise_orig.max_noise,
                (double)noise_retry_orig.max_noise, noise_orig.over_count,
                noise_retry_orig.over_count, tightened_bands, relaxed_bands,
                mode->trace_prefix, profile_reject_reason, trial_rollback_ok);
          }
        ...
        if (rollback_ok && have_legal_profile) {
          ...
          int const global_over_noise_ok =
              ...
              noise_orig.over_noise + SHORT_REDIST_GLOBAL_OVER_NOISE_SLOP;
          int const over_count_ok =
              best_noise_orig.over_count <=
              noise_orig.over_count + mode->over_count_slop;

          accept = best_bits <= MAX_BITS_PER_CHANNEL && attack_over_improved &&
                   attack_max_ok && global_max_ok && global_over_noise_ok &&
                   over_count_ok;
        ...
        if (cfg->analysis) {
          ...
          fprintf(stderr,
                  ...
                  "tightened_bands=%d relaxed_bands=%d "
                  "%s_reject_reason=%s rollback_ok=%d\n",
                  mode->trace_prefix, mode->trace_prefix, profiles_tried,
                  mode->trace_prefix, summary_profile, mode->trace_prefix,
                  accept, gfc->ov_enc.frame_number, gr2, ch2, psy_ch,
                  short_mask_relax_source_name(psy_ch), attack_win, old_bits,
                  summary_bits, (double)impulse_ratio, (double)tail_ratio,
                  mode->attack_profile_name, (double)old_attack_noise.max_noise,
                  (double)summary_attack_noise->max_noise,
                  (double)old_attack_noise.over_noise,
                  (double)summary_attack_noise->over_noise,
                  old_attack_noise.over_count, summary_attack_noise->over_count,
                  (double)noise_orig.max_noise,
                  (double)summary_noise_orig->max_noise, noise_orig.over_count,
                  summary_noise_orig->over_count, summary_tightened_bands,
                  summary_relaxed_bands, mode->trace_prefix, reject_reason,
                  rollback_ok);
        }
      }
    }
...
static void steady_tonal_protect_retry(
    lame_internal_flags *gfc, algo_t that_[2][2],
    const FLOAT l3_xmin[2][2][SFBMAX], int sfwork_[2][2][SFBMAX],
    int vbrsfmin_[2][2][SFBMAX], int max_nbits_ch[2][2], int ngr, int nch,
    int use_nbits_ch[2][2], int use_nbits_gr[2], int *use_nbits_fr,
  ...
  for (gr2 = 0; gr2 < ngr; ++gr2) {
    for (ch2 = 0; ch2 < nch; ++ch2) {
      ...
      calc_noise(cod_info, l3_xmin[gr2][ch2], old_distort, &noise_orig, 0);
      candidate =
          steady_tonal_candidate_select(gfc, cod_info, gr2, psy_ch, old_distort,
                                        &candidate_info, &candidate_reason);
      ...
      if (cfg->analysis) {
        fprintf(
            ...
            profiles_available, candidate_info.sfb_count,
            candidate_info.sfb_mask, candidate_info.first_sfb,
            candidate_info.last_sfb, (double)candidate_info.mean_tonality,
            (double)candidate_info.min_stability,
            (double)candidate_info.mean_ath_margin_db, old_bits,
...

      {
        ...
        for (profile_index = 0; profile_index < profiles_available;
             ++profile_index) {
          ...
          *cod_info = saved_gi;
          ...
          steady_tonal_build_xmin(redis_xmin, l3_xmin[gr2][ch2], cod_info,
                                  &candidate_info, profile, &tightened_bands);
          ...
          if (new_bits > MAX_BITS_PER_CHANNEL) {
            profile_reject_reason = "bit_limit";
                     ...
                     noise_orig.max_noise +
                         STEADY_TONAL_GLOBAL_MAX_NOISE_SLOP) {
            profile_reject_reason = "global_max_noise";
          } else if (noise_retry_orig.over_count >
          ...
          if (legal_profile &&
              (!have_legal_profile ||
               steady_tonal_profile_better(
                   &new_selected_failure, &noise_retry_orig, new_bits,
                   &best_selected_failure, &best_noise_orig, best_bits))) {
            have_legal_profile = 1;
            best_profile = profile_index;
            best_gi = *cod_info;
            memcpy(best_sfwork, sfwork_[gr2][ch2], sizeof(best_sfwork));
          ...
          *cod_info = saved_gi;
          ...
          trial_rollback_ok =
              ...
              fabsf(noise_restore_orig.over_noise - noise_orig.over_noise) <
                  1e-6f &&
              restore_selected_failure.over_count ==
                  old_selected_failure.over_count &&
              fabsf(restore_selected_failure.max_noise -
                    old_selected_failure.max_noise) < 1e-6f &&
              fabsf(restore_selected_failure.over_noise -
                    old_selected_failure.over_noise) < 1e-6f &&
              fabsf(restore_selected_failure.max_loss_db -
                    old_selected_failure.max_loss_db) < 1e-6f &&
              fabsf(restore_selected_failure.loss_db_sum -
                    old_selected_failure.loss_db_sum) < 1e-6f &&
              fabsf(restore_selected_failure.max_failure_db -
                    old_selected_failure.max_failure_db) < 1e-6f &&
              fabsf(restore_selected_failure.failure_db_sum -
                    old_selected_failure.failure_db_sum) < 1e-6f &&
              memcmp(gfc->l3_side.scfsi, saved_scfsi, sizeof(saved_scfsi)) == 0;
          ...
          if (cfg->analysis) {
            fprintf(
                ...
                short_mask_relax_source_name(psy_ch), profile_index,
                legal_profile, old_bits, new_bits, (double)profile->tighten_db,
                ...
                (double)noise_orig.max_noise,
                (double)noise_retry_orig.max_noise, noise_orig.over_count,
                noise_retry_orig.over_count, tightened_bands,
                profile_reject_reason, trial_rollback_ok);
          }
        ...
        if (rollback_ok && have_legal_profile) {
          ...
          int const global_over_noise_ok =
              best_noise_orig.over_noise <=
              noise_orig.over_noise + STEADY_TONAL_GLOBAL_OVER_NOISE_SLOP;
          int const over_count_ok =
              best_noise_orig.over_count <=
              noise_orig.over_count + STEADY_TONAL_OVERCOUNT_SLOP;

          accept = best_bits <= MAX_BITS_PER_CHANNEL &&
                   selected_failure_improved && selected_max_failure_ok &&
                   global_max_ok && global_over_noise_ok && over_count_ok;
        ...
        if (accept) {
          ...
          *use_nbits_fr += (best_nbits - old_nbits);
          if (steady_stats != 0) {
            steady_tonal_stats_note_accept(
                steady_stats, gfc->ov_enc.frame_number, cod_info, gr2, ch2,
                best_bits, &candidate_info, &old_selected_failure,
        ...
        if (cfg->analysis) {
          ...
          steady_tonal_failure_t const *summary_selected_failure =
              have_legal_profile ? &best_selected_failure
                                 : &old_selected_failure;
          calc_noise_result const *summary_noise_orig =
              have_legal_profile ? &best_noise_orig : &noise_orig;
          ...
          fprintf(
              ...
              "tightened_bands=%d "
              "steady_tonal_protect_reject_reason=%s rollback_ok=%d\n",
              profiles_tried, summary_profile, accept, gfc->ov_enc.frame_number,
              gr2, ch2, psy_ch, short_mask_relax_source_name(psy_ch),
              candidate_info.sfb_count, candidate_info.sfb_mask,
              candidate_info.first_sfb, candidate_info.last_sfb,
              (double)candidate_info.mean_tonality,
              (double)candidate_info.min_stability,
              (double)candidate_info.mean_ath_margin_db, old_bits, summary_bits,
              (double)old_selected_failure.max_noise,
...
inline static float vec_max_c(lamer_dsp const *dsp, const float *xr34,
                              unsigned int bw) {
...
static FLOAT calc_sfb_noise_x34(lamer_dsp const *dsp, const FLOAT *xr,
                                const FLOAT *xr34, unsigned int bw,
                                uint8_t sf) {
...
static uint8_t tri_calc_sfb_noise_x34(lamer_dsp const *dsp, const FLOAT *xr,
                                      const FLOAT *xr34, FLOAT l3_xmin,
...
static uint8_t guess_scalefac_x34(const algo_t *that, const FLOAT *xr,
                                  const FLOAT *xr34, FLOAT l3_xmin,
...
static uint8_t find_scalefac_x34(const algo_t *that, const FLOAT *xr,
                                 const FLOAT *xr34, FLOAT l3_xmin,
                                 unsigned int bw, uint8_t sf_min) {
  ...
  for (i = 0; i < 8; ++i) {
    ...
    if (sf <= sf_min) {
      ...
      uint8_t const bad =
          tri_calc_sfb_noise_x34(dsp, xr, xr34, l3_xmin, bw, sf, &did_it);
...
static int block_sf(algo_t *that, const FLOAT l3_xmin[SFBMAX],
  ...
  const char *const energy_above_cutoff =
      &that->cod_info->energy_above_cutoff[0];
...
static void quantize_x34(const algo_t *that) {
  ...
  unsigned int const max_nonzero_coeff =
      (unsigned int)cod_info->max_nonzero_coeff;
...
static const uint8_t max_range_long[SBMAX_l] = {15, 15, 15, 15, 15, 15, 15, 15,
                                                15, 15, 15, 7,  7,  7,  7,  7,
                                                7,  7,  7,  7,  7,  0};
...
static void set_subblock_gain(gr_info *cod_info, const int mingain_s[3],
                              int sf[]) {
...
*/
static void set_scalefacs(gr_info *cod_info, const int *vbrsfmin, int sf[],
...
static void long_block_constrain(const algo_t *that, const int vbrsf[SFBMAX],
                                 const int vbrsfmin[SFBMAX], int vbrmax) {
  gr_info *const cod_info = that->cod_info;
  lame_internal_flags const *const gfc = that->gfc;
  SessionConfig_t const *const cfg = &gfc->cfg;
...
static int quantizeAndCountBits(const algo_t *that) {
  quantize_x34(that);
  that->cod_info->part2_3_length =
      noquant_count_bits(that->gfc, that->cod_info, 0);
...
static int flattenDistribution(const int sfwork[SFBMAX], int sf_out[SFBMAX],
                               int dm, int k, int p) {
  unsigned int i, j;
  int x, sfmax = 0;
  if (dm > 0) {
...
static int tryThatOne(algo_t const *that, const int sftemp[SFBMAX],
                      const int vbrsfmin[SFBMAX], int vbrmax) {
  FLOAT const xrpow_max = that->cod_info->xrpow_max;
  int nbits = LARGE_BITS;
  that->alloc(that, sftemp, vbrsfmin, vbrmax);
...
int VBR_encode_frame(lame_internal_flags *gfc, const FLOAT xr34orig[2][2][576],
  ...
  if (gfc->cd_psy->experimental_short_transient_redistribute) {
    short_transient_redistribute_retry(
        gfc, that_, l3_xmin, sfwork_, vbrsfmin_, max_nbits_ch, ngr, nch,
        use_nbits_ch, use_nbits_gr, &use_nbits_fr, max_nbits_fr,
        &short_redist_mode_experimental);
  } else if (short_safe_transient_redistribute_allowed(gfc)) {
    short_transient_redistribute_retry(gfc, that_, l3_xmin, sfwork_, vbrsfmin_,
                                       max_nbits_ch, ngr, nch, use_nbits_ch,
                                       use_nbits_gr, &use_nbits_fr,
                                       max_nbits_fr, &short_redist_mode_safe);
  }
  if (safe_steady_tonal_protect_allowed(gfc)) {
    steady_tonal_protect_retry(gfc, that_, l3_xmin, sfwork_, vbrsfmin_,
                               max_nbits_ch, ngr, nch, use_nbits_ch,
                               use_nbits_gr, &use_nbits_fr, max_nbits_fr);
  }
  ...
  if (gfc->cd_psy->experimental_short_mask_relax &&
    ...
    for (gr2 = 0; gr2 < ngr; ++gr2) {
      for (ch2 = 0; ch2 < nch; ++ch2) {
        ...
        near_maxbits =
            (old_bits >= MAX_BITS_PER_CHANNEL - SHORT_MASK_RELAX_BIT_SLOP);
        ...
        if (cfg->analysis) {
          fprintf(
              ...
              candidate, gfc->ov_enc.frame_number, gr2, ch2, psy_ch,
              short_mask_relax_source_name(psy_ch), candidate_reason,
              (double)score_rel, final_mask, strong_short, near_maxbits,
              high_noise, old_bits, (double)noise_orig.max_noise,
              (double)noise_orig.over_noise, noise_orig.over_count,
...

        {
          ...
          calc_noise(cod_info, relaxed_xmin, old_distort, &noise_retry_relaxed,
                     0);
          ...
          if (cfg->analysis) {
            fprintf(
                ...
                accept, accept, gfc->ov_enc.frame_number, gr2, ch2, psy_ch,
                short_mask_relax_source_name(psy_ch), reject_reason,
                rollback_ok, (double)score_rel, final_mask, old_bits, new_bits,
                (double)noise_orig.max_noise,
                (double)noise_retry_orig.max_noise,
                (double)noise_orig.over_noise,
                (double)noise_retry_orig.over_noise, noise_orig.over_count,
                noise_retry_orig.over_count,
                (double)noise_retry_relaxed.max_noise,
                (double)noise_retry_relaxed.over_noise,
                noise_retry_relaxed.over_count, (double)SHORT_MASK_RELAX_DB,
                relaxed_bands);
          }
        }
      }
