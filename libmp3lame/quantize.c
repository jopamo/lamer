    ...
    if (cod_info->block_type != SHORT_TYPE) { /* NORM, START or STOP type, but not SHORT blocks */
        ...
        for (block = 0; block < 3; block++) {
            ...
            for (gsfb = PSFB12 - 1; gsfb >= 0 && !stop; gsfb--) {
                int const start = gfc->scalefac_band.s[12] * 3 + (gfc->scalefac_band.s[13] - gfc->scalefac_band.s[12]) * block + (gfc->scalefac_band.psfb12[gsfb] - gfc->scalefac_band.psfb12[0]);
                int const end = start + (gfc->scalefac_band.psfb12[gsfb + 1] - gfc->scalefac_band.psfb12[gsfb]);
                int j;
                FLOAT ath12;
...
static void init_outer_loop(lame_internal_flags const* gfc, gr_info* const cod_info) {
    ...
    if (cod_info->block_type == SHORT_TYPE) {
        ...
        if (cfg->samplerate_out <= 8000) {
            ...
            cod_info->psymax = cod_info->sfb_lmax + 3 * ((gfc->sv_qnt.sfb21_extra ? SBMAX_s : SBPSY_s) - cod_info->sfb_smin);
...
inline static int quant_compare(const int quant_comp, const calc_noise_result* const best, calc_noise_result* const calc, const gr_info* const gi, const FLOAT* distort) {
    /*
    ...
    switch (quant_comp) {
        ...
        case 0:
            better = calc->over_count < best->over_count || (calc->over_count == best->over_count && calc->over_noise < best->over_noise) ||
                     (calc->over_count == best->over_count && EQ(calc->over_noise, best->over_noise) && calc->tot_noise < best->tot_noise);
            break;
        ...
        case 4:
            better = (calc->max_noise <= 0.0 && best->max_noise > 0.2) ||
                     (calc->max_noise <= 0.0 && best->max_noise < 0.0 && best->max_noise > calc->max_noise - 0.2 && calc->tot_noise < best->tot_noise) ||
                     (calc->max_noise <= 0.0 && best->max_noise > 0.0 && best->max_noise > calc->max_noise - 0.2 && calc->tot_noise < best->tot_noise + best->over_noise) ||
                     (calc->max_noise > 0.0 && best->max_noise > -0.05 && best->max_noise > calc->max_noise - 0.1 && calc->tot_noise + calc->over_noise < best->tot_noise + best->over_noise) ||
            ...
            break;
        case 5:
            better = calc->over_noise < best->over_noise || (EQ(calc->over_noise, best->over_noise) && calc->tot_noise < best->tot_noise);
            break;
        case 6:
            better = calc->over_noise < best->over_noise ||
                     (EQ(calc->over_noise, best->over_noise) && (calc->max_noise < best->max_noise || (EQ(calc->max_noise, best->max_noise) && calc->tot_noise <= best->tot_noise)));
            break;
        case 7:
 ...
 *
 ********************************************************************/
inline static int balance_noise(lame_internal_flags* gfc, gr_info* const cod_info, FLOAT const* distort, FLOAT xrpow[576], int bRefine) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    int status;
...
static int outer_loop(lame_internal_flags* gfc,
                      gr_info* const cod_info,
        ...
        do {
            ...
            if (gfc->sv_qnt.sfb21_extra) {
                if (distort[cod_info_w.sfbmax] > 1.0)
                    break;
                if (cod_info_w.block_type == SHORT_TYPE && (distort[cod_info_w.sfbmax + 1] > 1.0 || distort[cod_info_w.sfbmax + 2] > 1.0))
                    break;
            }
             ...
             */
            while ((cod_info_w.part2_3_length = count_bits(gfc, xrpow, &cod_info_w, &prev_noise)) > huff_bits && cod_info_w.global_gain <= maxggain)
            ...
            if (best_noise_info.over_count == 0) {
                while ((cod_info_w.part2_3_length = count_bits(gfc, xrpow, &cod_info_w, &prev_noise)) > best_part2_3_length && cod_info_w.global_gain <= maxggain)
                    cod_info_w.global_gain++;
                ...
                *cod_info = cod_info_w;
                ...
                if (cfg->full_outer_loop == 0) {
                    ...
                    if ((cfg->noise_shaping_amp == 3) && bRefine && age > 30)
                        break;
                    if ((cfg->noise_shaping_amp == 3) && bRefine && (cod_info_w.global_gain - best_ggain_pass1) > 15)
                        break;
                }
            }
...
static void VBR_encode_granule(lame_internal_flags* gfc,
                               gr_info* const cod_info,
...
void VBR_old_iteration_loop(lame_internal_flags* gfc, const FLOAT pe[2][2], const FLOAT ms_ener_ratio[2], const III_psy_ratio ratio[2][2]) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    EncResult_t* const eov = &gfc->ov_enc;
    FLOAT l3_xmin[2][2][SFBMAX];
    ...
    analog_silence = VBR_old_prepare(gfc, pe, ms_ener_ratio, ratio, l3_xmin, frameBits, min_bits, max_bits, bands);
    ...
    for (;;) {
        ...
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            for (ch = 0; ch < cfg->channels_out; ch++) {
                ...
                VBR_encode_granule(gfc, cod_info, l3_xmin[gr][ch], xrpow, ch, min_bits[gr][ch], max_bits[gr][ch]);
...
static int VBR_new_prepare(lame_internal_flags* gfc, const FLOAT pe[2][2], const III_psy_ratio ratio[2][2], FLOAT l3_xmin[2][2][SFBMAX], int frameBits[16], int max_bits[2][2], int* max_resv) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    EncResult_t* const eov = &gfc->ov_enc;
...
void VBR_new_iteration_loop(lame_internal_flags* gfc, const FLOAT pe[2][2], const FLOAT ms_ener_ratio[2], const III_psy_ratio ratio[2][2]) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    EncResult_t* const eov = &gfc->ov_enc;
    FLOAT l3_xmin[2][2][SFBMAX];
...
static void calc_target_bits(lame_internal_flags* gfc, const FLOAT pe[2][2], FLOAT const ms_ener_ratio[2], int targ_bits_out[2][2], int* analog_silence_bits_out, int* max_frame_bits_out) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    EncResult_t* const eov = &gfc->ov_enc;
    ...
    if (gfc->ov_enc.mode_ext == MPG_MD_MS_LR)
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            reduce_side(targ_bits[gr], ms_ener_ratio[gr], mean_bits * cfg->channels_out, MAX_BITS_PER_GRANULE);
        }
...
void ABR_iteration_loop(lame_internal_flags* gfc, const FLOAT pe[2][2], const FLOAT ms_ener_ratio[2], const III_psy_ratio ratio[2][2]) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    EncResult_t* const eov = &gfc->ov_enc;
     ...
     */
    for (eov->bitrate_index = cfg->vbr_min_bitrate_index; eov->bitrate_index <= cfg->vbr_max_bitrate_index; eov->bitrate_index++) {
...
void CBR_iteration_loop(lame_internal_flags* gfc, const FLOAT pe[2][2], const FLOAT ms_ener_ratio[2], const III_psy_ratio ratio[2][2]) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    FLOAT l3_xmin[SFBMAX];
    FLOAT xrpow[576];
    int targ_bits[2];
