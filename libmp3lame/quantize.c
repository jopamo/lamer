...
static void init_xrpow_core_c(gr_info *const cod_info, FLOAT xrpow[576],
                              int upper, FLOAT *sum) {
  int i;
  FLOAT tmp;
  *sum = 0;
...
static int init_xrpow(lame_internal_flags *gfc, gr_info *const cod_info,
                      FLOAT xrpow[576]) {
  FLOAT sum = 0;
  int i;
...
static void psfb21_analogsilence(lame_internal_flags const *gfc,
  ...
  if (cod_info->block_type !=
      SHORT_TYPE) { /* NORM, START or STOP type, but not SHORT blocks */
    int gsfb;
    int stop = 0;
    ...
    for (block = 0; block < 3; block++) {
      ...
      for (gsfb = PSFB12 - 1; gsfb >= 0 && !stop; gsfb--) {
        ...
        int const end = start + (gfc->scalefac_band.psfb12[gsfb + 1] -
  ...
  if (cod_info->block_type == SHORT_TYPE) {
    ...
    if (cfg->samplerate_out <= 8000) {
      ...
      cod_info->psymax = cod_info->sfb_lmax +
                         3 * ((gfc->sv_qnt.sfb21_extra ? SBMAX_s : SBPSY_s) -
                              cod_info->sfb_smin);
      cod_info->sfbmax =
...
static int bin_search_StepSize(lame_internal_flags *const gfc,
                               gr_info *const cod_info, int desired_rate,
                               const int ch, const FLOAT xrpow[576]) {
  int nBits;
...
inline static int quant_compare(const int quant_comp,
                                ...
                                const gr_info *const gi, const FLOAT *distort) {
  /*
  ...
  case 9: {
    if (best->over_count > 0) {
      ...
      /* no distorted sfb */
      better = ((calc->max_noise < 0) && ((calc->max_noise * 10 + calc->bits) <=
...
static void amp_scalefac_bands(lame_internal_flags *gfc,
                               gr_info *const cod_info, FLOAT const *distort,
                               FLOAT xrpow[576], int bRefine) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  int j, sfb;
...
static int inc_subblock_gain(const lame_internal_flags *const gfc,
                             gr_info *const cod_info, FLOAT xrpow[576]) {
  int sfb, window;
  ...
  for (window = 0; window < 3; window++) {
    ...
    for (sfb = cod_info->sfb_lmax + window; sfb < cod_info->sfbdivide;
         sfb += 3) {
      if (s1 < scalefac[sfb])
        s1 = scalefac[sfb];
    }
...
static int outer_loop(lame_internal_flags *gfc, gr_info *const cod_info,
  ...
  while (!bEndOfSearch) {
    ...
    do {
       ...
       */
      while ((cod_info_w.part2_3_length = count_bits(
                  gfc, xrpow, &cod_info_w, &prev_noise)) > huff_bits &&
      ...
      better = quant_compare(better, &best_noise_info, &noise_info, &cod_info_w,
                             distort);
...
static void VBR_encode_granule(
    lame_internal_flags *gfc, gr_info *const cod_info,
...
static int VBR_old_prepare(lame_internal_flags *gfc, const FLOAT pe[2][2],
                           FLOAT const ms_ener_ratio[2],
                           const III_psy_ratio ratio[2][2],
                           FLOAT l3_xmin[2][2][SFBMAX], int frameBits[16],
...
static void bitpressure_strategy(lame_internal_flags const *gfc,
                                 ...
                                 const int min_bits[2][2], int max_bits[2][2]) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  int gr, ch, sfb;
  for (gr = 0; gr < cfg->mode_gr; gr++) {
    for (ch = 0; ch < cfg->channels_out; ch++) {
...
void VBR_old_iteration_loop(lame_internal_flags *gfc, const FLOAT pe[2][2],
  ...
  analog_silence = VBR_old_prepare(gfc, pe, ms_ener_ratio, ratio, l3_xmin,
                                   frameBits, min_bits, max_bits, bands);
  ...
  for (;;) {
    ...
    for (gr = 0; gr < cfg->mode_gr; gr++) {
      for (ch = 0; ch < cfg->channels_out; ch++) {
        ...
        VBR_encode_granule(gfc, cod_info, l3_xmin[gr][ch], xrpow, ch,
                           min_bits[gr][ch], max_bits[gr][ch]);

        /*  do the 'substep shaping'
         */
        if (gfc->sv_qnt.substep_shaping & 1) {
          trancate_smallspectrums(gfc, &l3_side->tt[gr][ch], l3_xmin[gr][ch],
                                  xrpow);
        }
    ...
    for (; eov->bitrate_index < cfg->vbr_max_bitrate_index;
         eov->bitrate_index++) {
...
static int VBR_new_prepare(lame_internal_flags *gfc, const FLOAT pe[2][2],
                           const III_psy_ratio ratio[2][2],
                           FLOAT l3_xmin[2][2][SFBMAX], int frameBits[16],
                           int max_bits[2][2], int *max_resv) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  EncResult_t *const eov = &gfc->ov_enc;
...
static void calc_target_bits(lame_internal_flags *gfc, const FLOAT pe[2][2],
                             FLOAT const ms_ener_ratio[2],
                             int targ_bits_out[2][2],
                             int *analog_silence_bits_out,
  ...
  if (gfc->ov_enc.mode_ext == MPG_MD_MS_LR)
    for (gr = 0; gr < cfg->mode_gr; gr++) {
      reduce_side(targ_bits[gr], ms_ener_ratio[gr],
                  mean_bits * cfg->channels_out, MAX_BITS_PER_GRANULE);
    }
...
void ABR_iteration_loop(lame_internal_flags *gfc, const FLOAT pe[2][2],
  ...
  calc_target_bits(gfc, pe, ms_ener_ratio, targ_bits, &analog_silence_bits,
                   &max_frame_bits);
