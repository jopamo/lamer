...
inline static void writeheader(lame_internal_flags* gfc, int val, int j) {
    ...
    while (j > 0) {
        ...
        esv->header[esv->h_ptr].buf[ptr >> 3] |= ((unsigned int)val >> j) << (8 - (ptr & 7) - k);
        ptr += k;
    }
...
/*
  ...
  */
inline static int Huffmancode(lame_internal_flags* const gfc, const unsigned int tableindex, int start, int end, gr_info const* gi) {
...
int format_bitstream(lame_internal_flags* gfc) {
    ...
    if ((l3_side->main_data_begin * 8) != esv->ResvSize) {
        ERRORF(gfc,
               ...
               "bitsperframe:             %i \n",
               8 * l3_side->main_data_begin, esv->ResvSize, l3_side->resvDrain_post, l3_side->resvDrain_pre, 8 * cfg->sideinfo_len, bits - l3_side->resvDrain_post - 8 * cfg->sideinfo_len, bits,
               bits % 8, bitsPerFrame);

        ERRORF(gfc, "This is a fatal error.  It has several possible causes:");
        ERRORF(gfc,
               "90%%  LAME compiled with buggy version of gcc using advanced "
               "optimizations");
