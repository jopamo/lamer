...
static int calcFrameLength(SessionConfig_t const *const cfg, int kbps,
                           int pad) {
...
int get_max_frame_buffer_size_by_constraint(SessionConfig_t const *cfg,
                                            int constraint) {
  int maxmp3buf = 0;
  if (cfg->avg_bitrate > 320) {
    ...
    if (cfg->samplerate_out < 16000) {
      max_kbps = bitrate_table[cfg->version]
                              [8]; /* default: allow 64 kbps (MPEG-2.5) */
    } else {
    ...
    case MDB_DEFAULT:
       ...
       * of using 8*960. */
      /* All mp3 decoders should have enough buffer to handle this value: size
       * of a 320kbps 32kHz frame */
      maxmp3buf = 8 * 1440;
      break;
    case MDB_STRICT_ISO:
...
#endif
  memcpy(&bs->buf[bs->buf_byte_idx], esv->header[esv->w_ptr].buf,
         cfg->sideinfo_len);
...
inline static void drain_into_ancillary(lame_internal_flags *gfc,
                                        int remainingBits) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  EncStateVar_t *const esv = &gfc->sv_enc;
  int i;
...
inline static void writeheader(lame_internal_flags *gfc, int val, int j) {
  ...
  while (j > 0) {
    ...
    esv->header[esv->h_ptr].buf[ptr >> 3] |= ((unsigned int)val >> j)
                                             << (8 - (ptr & 7) - k);
    ptr += k;
  }
...
static inline void fixup_table_select(lame_internal_flags *gfc, int *tbl) {
  if (*tbl == 14)
    *tbl = 16;
...
inline static int huffman_coder_count1(lame_internal_flags *gfc,
                                       gr_info const *gi) {
  /* Write count1 area */
...
/*
  ...
  */
inline static int Huffmancode(lame_internal_flags *const gfc,
                              const unsigned int tableindex, int start, int end,
                              gr_info const *gi) {
...
int format_bitstream(lame_internal_flags *gfc) {
  ...
  if ((l3_side->main_data_begin * 8) != esv->ResvSize) {
    ERRORF(gfc,
           ...
           "bitsperframe:             %i \n",
           8 * l3_side->main_data_begin, esv->ResvSize, l3_side->resvDrain_post,
           l3_side->resvDrain_pre, 8 * cfg->sideinfo_len,
           bits - l3_side->resvDrain_post - 8 * cfg->sideinfo_len, bits,
           bits % 8, bitsPerFrame);

    ERRORF(gfc, "This is a fatal error.  It has several possible causes:");
    ERRORF(gfc, "90%%  LAME compiled with buggy version of gcc using advanced "
                "optimizations");
  ...
  if (gfc->bs.totbit > 1000000000) {
    /* to avoid totbit overflow, (at 8h encoding at 128kbs) lets reset bit
     * counter */
    int i;
    for (i = 0; i < MAX_HEADER_BUF; ++i)
...
static int do_copy_buffer(lame_internal_flags *gfc, unsigned char *buffer,
                          int size) {
