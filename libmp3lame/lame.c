...
void lame_print_config(const lame_global_flags *gfp) {
  ...
  MSGF(gfc, "LAME %s %s (%s)\n", get_lame_version(), get_lame_os_bitness(),
       get_lame_url());
  ...
  if (gfc->CPU_features.MMX || gfc->CPU_features.AMD_3DNow ||
    ...
    if (gfc->CPU_features.AMD_3DNow) {
      concatSep(text, ", ",
                (fft_asm_used == 1) ? "3DNow! (ASM used)" : "3DNow!");
    }
  ...
  if (cfg->free_format) {
    ...
    if (cfg->avg_bitrate > 320) {
      MSGF(gfc, "Warning: many decoders cannot handle free format bitrates "
...
void lame_print_internals(const lame_global_flags *gfp) {
  ...
  MSGF(
      ...
      10 * log10(gfc->sv_qnt.longfact[0]), 10 * log10(gfc->sv_qnt.longfact[7]),
...
void lame_bitrate_stereo_mode_hist(const lame_global_flags *gfp,
...
void lame_bitrate_block_type_hist(const lame_global_flags *gfp,
