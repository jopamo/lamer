...
*/
const int nr_of_sfb_block[6][3][4] = {{{6, 5, 5, 5}, {9, 9, 9, 9}, {6, 9, 9, 9}},      {{6, 5, 7, 3}, {9, 9, 12, 6}, {6, 9, 12, 6}}, {{11, 10, 0, 0}, {18, 18, 0, 0}, {15, 18, 0, 0}},
                                      {{7, 7, 7, 0}, {12, 12, 12, 0}, {6, 15, 12, 0}}, {{6, 6, 6, 3}, {12, 9, 9, 6}, {6, 12, 9, 6}}, {{8, 8, 5, 0}, {15, 12, 9, 0}, {6, 18, 9, 0}}};
...
static float const payload_long[2][4] = {{-0.000f, -0.000f, -0.000f, +0.000f}, {-0.500f, -0.250f, -0.025f, +0.500f}};
static float const payload_short[2][4] = {{-0.000f, -0.000f, -0.000f, +0.000f}, {-2.000f, -1.000f, -0.050f, +0.500f}};
 ...
 ************************************************************************/
int on_pe(lame_internal_flags* gfc, const FLOAT pe[2][2], int targ_bits[2], int mean_bits, int gr, int cbr) {
    SessionConfig_t const* const cfg = &gfc->cfg;
...
int calc_xmin(lame_internal_flags const* gfc, III_psy_ratio const* const ratio, gr_info* const cod_info, FLOAT* pxmin) {
    SessionConfig_t const* const cfg = &gfc->cfg;
...
int calc_noise(gr_info const* const cod_info, FLOAT const* l3_xmin, FLOAT* distort, calc_noise_result* const res, calc_noise_data* prev_noise) {
    ...
    for (sfb = 0; sfb < psymax; sfb++) {
        ...
        int const s = global_gain - scalefac_value * (1 << scalefac_shift) - subblock_gain[window[sfb]] * 8;
...
static void set_pinfo(lame_internal_flags const* gfc, gr_info* const cod_info, const III_psy_ratio* const ratio, const int gr, const int ch) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    int sfb, sfb2;
    ...
    if (cod_info->block_type == SHORT_TYPE) {
        ...
        for (sfb = cod_info->sfb_smin; sfb < SBMAX_s; sfb++) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                gfc->pinfo->thr_s[gr][ch][3 * sfb + i] = en1 * Max(en0 * ratio->thm.s[sfb][i], gfc->ATH->s[sfb]);
