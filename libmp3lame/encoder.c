...
static void lame_encode_frame_init(lame_internal_flags *gfc,
                                   const sample_t *const inbuf[2]) {
  SessionConfig_t const *const cfg = &gfc->cfg;
...
int lame_encode_mp3_frame(                          /* Output */
...

  {
    ...
    const sample_t *bufp[2] = {
        0, 0}; /* address of beginning of left & right granule */
    int blocktype[2];
    ...
    for (gr = 0; gr < cfg->mode_gr; gr++) {
      ...
      ret = L3psycho_anal_vbr(gfc, bufp, gr, masking_LR, masking_MS, pe[gr],
                              pe_MS[gr], tot_ener[gr], blocktype);
      if (ret != 0)
        return -4;
  ...
  if (cfg->analysis && gfc->pinfo != NULL) {
    for (gr = 0; gr < cfg->mode_gr; gr++) {
      for (ch = 0; ch < cfg->channels_out; ch++) {
        ...
        memcpy(gfc->pinfo->xr[gr][ch], &gfc->l3_side.tt[gr][ch].xr[0],
               sizeof(FLOAT) * 576);
      ...
      }
      {
        ...
        int const n_chn_psy =
            (cfg->mode == JOINT_STEREO) ? 4 : cfg->channels_out;
        int psy_ch;
        for (psy_ch = 0; psy_ch < n_chn_psy; ++psy_ch) {
          ...
          fprintf(
              ...
              psy_ch < 2 ? "LR" : "MS", gfc->pinfo->blocktype[gr][out_ch],
              trans->score_rel[psy_ch], trans->score[psy_ch],
              trans->impulse_ratio[psy_ch], trans->tail_ratio[psy_ch],
              (int)trans->pos[psy_ch], (unsigned)trans->raw_mask[psy_ch],
              (unsigned)trans->final_mask[psy_ch],
              (unsigned)trans->suppressed_mask[psy_ch],
              (unsigned)trans->count[psy_ch], (int)trans->last_attack[psy_ch],
  ...
  if (cfg->vbr == vbr_off || cfg->vbr == vbr_abr) {
    static FLOAT const fircoef[9] = {
        -0.0207887 * 5, -0.0378413 * 5,  -0.0432472 * 5,
        -0.031183 * 5,  7.79609e-18 * 5, 0.0467745 * 5,
    ...
    for (i = 0; i < 9; i++)
      f +=
          (gfc->sv_enc.pefirbuf[i] + gfc->sv_enc.pefirbuf[18 - i]) * fircoef[i];
  ...
  case vbr_abr:
    ...
    break;
  case vbr_rh:
    VBR_old_iteration_loop(gfc, (const FLOAT(*)[2])pe_use, ms_ener_ratio,
                           masking);
    break;
  case vbr_mtrh:
    VBR_new_iteration_loop(gfc, (const FLOAT(*)[2])pe_use, ms_ener_ratio,
                           masking);
    break;
  }
      ...
      for (gr_a = 0; gr_a < cfg->mode_gr; ++gr_a) {
        ...
        for (psy_ch_a = 0; psy_ch_a < n_chn_psy_alloc; ++psy_ch_a) {
          ...
          if (cod_info_a->block_type == SHORT_TYPE &&
            ...
            int add_bits_ch = (int)(gfc->pinfo->mean_bits * pe_val / 700.0 -
                                    gfc->pinfo->mean_bits);
            ...
            fprintf(stderr,
                    ...
                    (double)pe_val, gfc->pinfo->mean_bits + add_bits_ch,
                    add_bits_ch, gfc->pinfo->mean_bits, gfc->pinfo->resvsize,
                    gfc->sv_enc.ResvMax, (int)cod_info_a->part2_3_length,
                    (int)cod_info_a->part2_length, (int)cod_info_a->big_values,
        ...
        for (psy_ch_c = 0; psy_ch_c < n_chn_psy_alloc; ++psy_ch_c) {
          ...
          if (cod_info_c->block_type == SHORT_TYPE &&
            ...
            fprintf(stderr,
                    ...
                    gfc->ov_enc.frame_number, gr_c, out_ch_c, psy_ch_c, class_c,
                    (double)score_c, part2_3_c, (double)max_noise_c, over_c);
          }
        }
      }
    }
