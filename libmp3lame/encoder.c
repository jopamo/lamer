...
static char const* policy_mask_name(unsigned char mask) {
    if (mask & 0x02u) {
        return "forced";
    }
    if (mask & 0x04u) {
...
static void lame_encode_frame_init(lame_internal_flags* gfc, const sample_t* const inbuf[2]) {
    SessionConfig_t const* const cfg = &gfc->cfg;
...
int lame_encode_mp3_frame(                          /* Output */
...

    {
        ...
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            ...
            ret = L3psycho_anal_vbr(gfc, bufp, gr, masking_LR, masking_MS, pe[gr], pe_MS[gr], tot_ener[gr], blocktype);
            if (ret != 0)
                return -4;
        ...
        for (gr = 0; gr < cfg->mode_gr; gr++) {
            for (ch = 0; ch < cfg->channels_out; ch++) {
                ...
                if (gfc->ov_enc.mode_ext == MPG_MD_MS_LR) {
                    ...
                    memcpy(gfc->pinfo->energy[gr][ch], gfc->pinfo->energy[gr][ch + 2], sizeof(gfc->pinfo->energy[gr][ch]));
                }
            }
            {
                ...
                for (psy_ch = 0; psy_ch < n_chn_psy; ++psy_ch) {
                    ...
                    fprintf(stderr,
                            ...
                            "blocktype=%d score_rel=%.2f score=%.0f impulse_ratio=%.2f "
                            "tail_ratio=%.2f pos=%d "
                            "raw_mask=0x%02x final_mask=0x%02x suppressed_mask=0x%02x "
                            "count=%u "
                            "last_attack=%d uselong_before_policy=%u uselong_after_policy=%u "
                            "policy=%s\n",
                            gfc->ov_enc.frame_number, gr, psy_ch, out_ch, psy_ch < 2 ? "LR" : "MS", gfc->pinfo->blocktype[gr][out_ch], trans->score_rel[psy_ch], trans->score[psy_ch],
                            trans->impulse_ratio[psy_ch], trans->tail_ratio[psy_ch], (int)trans->pos[psy_ch], (unsigned)trans->raw_mask[psy_ch], (unsigned)trans->final_mask[psy_ch],
                            (unsigned)trans->suppressed_mask[psy_ch], (unsigned)trans->count[psy_ch], (int)trans->last_attack[psy_ch], (unsigned)trans->uselong_before_policy[out_ch],
                            (unsigned)trans->uselong_after_policy[out_ch], policy_mask_name(trans->policy_mask[out_ch]));
                }
            }
        }
    }
    ...
    if (cfg->vbr == vbr_off || cfg->vbr == vbr_abr) {
        static FLOAT const fircoef[9] = {-0.0207887 * 5, -0.0378413 * 5, -0.0432472 * 5, -0.031183 * 5, 7.79609e-18 * 5, 0.0467745 * 5, 0.10091 * 5, 0.151365 * 5, 0.187098 * 5};
        ...
        /* short-block allocation diagnostics */
        {
            ...
            for (gr_a = 0; gr_a < cfg->mode_gr; ++gr_a) {
                ...
                for (psy_ch_a = 0; psy_ch_a < n_chn_psy_alloc; ++psy_ch_a) {
                    ...
                    if (cod_info_a->block_type == SHORT_TYPE && trans_a->score_rel[psy_ch_a] >= 1.0 && trans_a->final_mask[psy_ch_a] != 0) {
                        ...
                        fprintf(stderr,
                                ...
                                "over=%d tot_noise=%.2f max_noise=%.2f over_noise=%.2f "
                                "over_ssd=%d "
                                "br_idx=%d mode_ext=%d\n",
                                gfc->ov_enc.frame_number, gr_a, out_ch_a, psy_ch_a, (double)pe_val, gfc->pinfo->mean_bits + add_bits_ch, add_bits_ch, gfc->pinfo->mean_bits, gfc->pinfo->resvsize,
                                gfc->sv_enc.ResvMax, (int)cod_info_a->part2_3_length, (int)cod_info_a->part2_length, (int)cod_info_a->big_values, (int)cod_info_a->global_gain,
                                (int)cod_info_a->subblock_gain[0], (int)cod_info_a->subblock_gain[1], (int)cod_info_a->subblock_gain[2], (int)cod_info_a->scalefac_scale,
                                gfc->pinfo->over[gr_a][out_ch_a], (double)gfc->pinfo->tot_noise[gr_a][out_ch_a], (double)gfc->pinfo->max_noise[gr_a][out_ch_a],
                                (double)gfc->pinfo->over_noise[gr_a][out_ch_a], gfc->pinfo->over_SSD[gr_a][out_ch_a], gfc->ov_enc.bitrate_index, gfc->ov_enc.mode_ext);
                    }
                }
            }
        }
        /* short-block frame classifier */
        {
            ...
            for (gr_c = 0; gr_c < cfg->mode_gr; ++gr_c) {
                ...
                for (psy_ch_c = 0; psy_ch_c < n_chn_psy_alloc; ++psy_ch_c) {
                    ...
                    if (cod_info_c->block_type == SHORT_TYPE && trans_c->final_mask[psy_ch_c] != 0 && trans_c->score_rel[psy_ch_c] > 0.5f) {
                        ...
                        fprintf(stderr,
                                ...
                                gfc->ov_enc.frame_number, gr_c, out_ch_c, psy_ch_c, class_c, (double)score_c, part2_3_c, (double)max_noise_c, over_c);
                    }
                }
            }
        }
