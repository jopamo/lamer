...
static void save_gain_values(lame_internal_flags *gfc) {
  ...
  if (cfg->findReplayGain) {
    ...
    if (NEQ(RadioGain, GAIN_NOT_ENOUGH_SAMPLES)) {
      rov->RadioGain =
          (int)floor(RadioGain * 10.0 + 0.5); /* round to nearest */
    } else {
      rov->RadioGain = 0;
    }
  }
  ...
  if (cfg->findPeakSample) {
    ...
    if (rov->noclipGainChange > 0) { /* clipping occurs */
      rov->noclipScale = floor((32767.0f / rov->PeakSample) * 100.0f) /
                         100.0f; /* round down */
    } else                       /* no clipping */
...
/*****************************************************************
 ...
 with no errors.
*********************************************************************/
int lame_encode_flush_nogap(lame_global_flags *gfp, unsigned char *mp3buffer,
                            int mp3buffer_size) {
  int rc = -3;
...
static int calc_mp3buffer_size_remaining(int mp3buffer_size, int mp3count) {
  /* if user specifed buffer size = 0, dont check size */
  if (mp3buffer_size == 0)
    return INT_MAX;
...
int lame_encode_flush(lame_global_flags *gfp, unsigned char *mp3buffer,
                      int mp3buffer_size) {
  ...
  if (is_resampling_necessary) {
    ...
    /* delay due to resampling; needs to be fixed, if resampling code gets
     * changed */
    samples_to_encode += 16. / resample_ratio;
  }
  end_padding =
      pcm_samples_per_frame - (samples_to_encode % pcm_samples_per_frame);
  if (end_padding < 576)
  ...
  while (frames_left > 0 && imp3 >= 0) {
    ...
    mp3buffer_size_remaining =
        calc_mp3buffer_size_remaining(mp3buffer_size, mp3count);

    /* send in a frame of 0 padding until all internal sample buffers
     * are flushed
     */
    imp3 = lame_encode_buffer(gfp, buffer[0], buffer[1], bunch, mp3buffer,
                              mp3buffer_size_remaining);
    if (imp3 > 0) {
      mp3buffer += imp3;
      mp3count += imp3;
