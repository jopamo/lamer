...
static void apply_default_vbr_floor(lame_global_flags *gfp,
...
static void optimum_bandwidth(double *const lowerlimit,
  ...
  const band_pass_t freq_map[] = {
      {8, 2000},    {16, 3700},   {24, 3900},   {32, 5500},   {40, 7000},
      {48, 7500},   {56, 10000},  {64, 11000},  {80, 13500},  {96, 15100},
      {112, 15600}, {128, 17000}, {160, 17500}, {192, 18600}, {224, 19400},
      {256, 19700}, {320, 20500}};
...
int lame_init_params(lame_global_flags *gfp) {
  ...
  if (cfg->vbr == vbr_off && gfp->VBR_mean_bitrate_kbps != 128 &&
      gfp->brate == 0)
  ...
  if (cfg->vbr == vbr_off && gfp->brate == 0) {
    ...
    if (EQ(gfp->compression_ratio, 0))
      gfp->compression_ratio =
          11.025; /* rate to compress a CD down to exactly 128000 bps */
  }
  ...
  if (cfg->vbr == vbr_off && gfp->compression_ratio > 0) {
     ...
     */
    gfp->brate = gfp->samplerate_out * 16 * cfg->channels_out /
    ...
    if (!cfg->free_format) /* for non Free Format find the nearest allowed
                              bitrate */
      gfp->brate =
          FindNearestBitrate(gfp->brate, cfg->version, gfp->samplerate_out);
  }
  ...
  if (gfp->lowpassfreq == 0) {
    ...
    case vbr_rh: {
      int const x[11] = {19500, 19000, 18600, 18000, 17500, 16000,
    ...
    case vbr_mtrh: {
      int const x[11] = {24000, 19500, 18500, 18000, 17500, 17000,
                         16500, 15600, 15200, 7230,  3950};
      if (0 <= gfp->VBR_q && gfp->VBR_q <= 9) {
        double a = x[gfp->VBR_q], b = x[gfp->VBR_q + 1], m = gfp->VBR_q_frac;
        lowpass = linear_int(a, b, m);
      } else {
    ...
    default: {
      int const x[11] = {19500, 19000, 18500, 18000, 17500, 16500,
  ...
  if (gfp->samplerate_out == 0) {
    ...
    gfp->samplerate_out =
        optimum_samplefreq((int)gfp->lowpassfreq, gfp->samplerate_in);
  }
  if (cfg->vbr == vbr_mtrh) {
  ...
  if (cfg->vbr == vbr_abr) {
    gfp->compression_ratio = gfp->samplerate_out * 16 * cfg->channels_out /
  ...
  case vbr_abr:
    gfp->compression_ratio = cfg->samplerate_out * 16 * cfg->channels_out /
                             (1.e3 * gfp->VBR_mean_bitrate_kbps);
    break;
  default:
    gfp->compression_ratio =
  ...
  if (cfg->vbr == vbr_off) {
    if (cfg->free_format) {
      ...
      gfp->brate =
          FindNearestBitrate(gfp->brate, cfg->version, cfg->samplerate_out);
  ...
  j = cfg->samplerate_index + (3 * cfg->version) +
      6 * (cfg->samplerate_out < 16000);
  for (i = 0; i < SBMAX_l + 1; i++)
    gfc->scalefac_band.l[i] = sfBandIndex[j].l[i];

  for (i = 0; i < PSFB21 + 1; i++) {
    int const size =
        (gfc->scalefac_band.l[22] - gfc->scalefac_band.l[21]) / PSFB21;
  ...
  for (i = 0; i < PSFB12 + 1; i++) {
    int const size =
        (gfc->scalefac_band.s[13] - gfc->scalefac_band.s[12]) / PSFB12;
  ...
  if (cfg->vbr != vbr_off) { /* choose a min/max bitrate for VBR */
    ...
    cfg->vbr_min_bitrate_index =
        1; /* default: allow   8 kbps (MPEG-2) or  32 kbps (MPEG-1) */
    cfg->vbr_max_bitrate_index =
        14; /* default: allow 160 kbps (MPEG-2) or 320 kbps (MPEG-1) */
    if (cfg->samplerate_out < 16000)
    ...
    if (gfp->VBR_min_bitrate_kbps) {
      ...
      cfg->vbr_min_bitrate_index = BitrateIndex(
          gfp->VBR_min_bitrate_kbps, cfg->version, cfg->samplerate_out);
    ...
    if (gfp->VBR_max_bitrate_kbps) {
      ...
      cfg->vbr_max_bitrate_index = BitrateIndex(
          gfp->VBR_max_bitrate_kbps, cfg->version, cfg->samplerate_out);
