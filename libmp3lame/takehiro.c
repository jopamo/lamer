...
static void quantize_xrpow(const FLOAT* xp, int* pi, FLOAT istep, gr_info const* const cod_info, calc_noise_data const* prev_noise) {
    ...
    for (sfb = 0; sfb <= sfbmax; sfb++) {
        ...
        if (prev_data_use || cod_info->block_type == NORM_TYPE) {
            step = cod_info->global_gain - ((cod_info->scalefac[sfb] + (cod_info->preflag ? pretab[sfb] : 0)) << (cod_info->scalefac_scale + 1)) - cod_info->subblock_gain[cod_info->window[sfb]] * 8;
        }
        ...
        if (prev_data_use && (prev_noise->step[sfb] == step)) {
            ...
            if ((j + cod_info->width[sfb]) > cod_info->max_nonzero_coeff) {
                ...
                memset(&pi[cod_info->max_nonzero_coeff], 0, sizeof(int) * (576 - cod_info->max_nonzero_coeff));
                l = usefullsize;
            ...
            if (prev_noise && prev_noise->sfb_count1 > 0 && sfb >= prev_noise->sfb_count1 && prev_noise->step[sfb] > 0 && step >= prev_noise->step[sfb]) {
                if (accumulate) {
                    quantize_lines_xrpow(accumulate, istep, acc_xp, acc_iData);
                    accumulate = 0;
                    acc_iData = iData;
...
static const count_fnc count_fncs[] = {&count_bit_null,        &count_bit_noESC,       &count_bit_noESC_from2, &count_bit_noESC_from2, &count_bit_noESC_from3, &count_bit_noESC_from3,
                                       &count_bit_noESC_from3, &count_bit_noESC_from3, &count_bit_noESC_from3, &count_bit_noESC_from3, &count_bit_noESC_from3, &count_bit_noESC_from3,
...
static int choose_table_nonMMX_with_max(const int* ix, const int* const end, unsigned int max, int* const _s) {
...
static void long_block_region_max_cache_init(const lame_internal_flags* const gfc, const int* const ix, int const bigv, long_block_region_max_cache_t* const cache) {
...
inline static void best_huffman_divide_cached(const lame_internal_flags* const gfc, gr_info* const gi, const long_block_region_max_cache_t* const cached_max);
...
inline static void recalc_divide_init(const lame_internal_flags* const gfc,
    ...
    for (r0 = 0; r0 < 16; r0++) {
        ...
        for (r1 = 0; r1 < 8; r1++) {
            ...
            bits = r0bits;
            r1t = choose_table_nonMMX_with_max(ix + a1, ix + a2, max_cache->range_max[r0 + 1][r0 + r1 + 2], &bits);
...
inline static void recalc_divide_sub(const lame_internal_flags* const gfc,
    ...
    for (r2 = 2; r2 < SBMAX_l + 1; r2++) {
        ...
        r2t = choose_table_nonMMX_with_max(ix + a2, ix + bigv, max_cache->range_max[r2][max_cache->band_count], &bits);
        if (gi->part2_3_length <= bits)
            continue;
...
inline static void best_huffman_divide_cached(const lame_internal_flags* const gfc, gr_info* const gi, const long_block_region_max_cache_t* const cached_max) {
    ...
    if (cod_info2.block_type == NORM_TYPE)
        ...
        if (a1 > 0)
            cod_info2.table_select[0] = choose_table_nonMMX(ix, ix + a1, (int*)&cod_info2.part2_3_length);
        if (i > a1)
            cod_info2.table_select[1] = choose_table_nonMMX(ix + a1, ix + i, (int*)&cod_info2.part2_3_length);
...
void best_scalefac_store(const lame_internal_flags* gfc, const int gr, const int ch, III_side_info_t* const l3_side) {
    ...
    if (cfg->mode_gr == 2 && gr == 1 && l3_side->tt[0][ch].block_type != SHORT_TYPE && l3_side->tt[1][ch].block_type != SHORT_TYPE) {
...
static const int scale_short[16] = {0, 18, 36, 54, 54, 36, 54, 72, 54, 72, 90, 72, 90, 108, 108, 126};

/* 17*slen1_tab[i] + 18*slen2_tab[i] */
static const int scale_mixed[16] = {0, 18, 36, 54, 51, 35, 53, 71, 52, 70, 88, 69, 87, 105, 104, 122};
...
*/
static const int max_range_sfac_tab[6][4] = {{15, 15, 7, 7}, {15, 15, 7, 0}, {7, 3, 0, 0}, {15, 31, 31, 0}, {7, 7, 7, 0}, {3, 3, 0, 0}};
