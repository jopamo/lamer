...
static void quantize_lines_xrpow_01(unsigned int l, FLOAT istep,
                                    const FLOAT *xr, int *ix) {
...
static void quantize_lines_xrpow(unsigned int l, FLOAT istep, const FLOAT *xr,
                                 int *ix) {
...
static void quantize_xrpow(const FLOAT *xp, int *pi, FLOAT istep,
  ...
  prev_data_use =
      (prev_noise && (cod_info->global_gain == prev_noise->global_gain));
  ...
  for (sfb = 0; sfb <= sfbmax; sfb++) {
    ...
    if (prev_data_use && (prev_noise->step[sfb] == step)) {
      ...
      if (prev_noise && prev_noise->sfb_count1 > 0 &&
          sfb >= prev_noise->sfb_count1 && prev_noise->step[sfb] > 0 &&
...
static int count_bit_ESC(const int *ix, const int *const end, int t1,
                         const int t2, unsigned int *const s) {
  /* ESC-table is used */
...
static int count_bit_noESC(const int *ix, const int *end, int mx,
                           unsigned int *s) {
  /* No ESC-words */
  unsigned int sum1 = 0;
...
static const int huf_tbl_noESC[] = {1,  2,  5,  7,  7,  10, 10, 13,
                                    13, 13, 13, 13, 13, 13, 13};

static int count_bit_noESC_from2(const int *ix, const int *end, int max,
                                 unsigned int *s) {
...
inline static int count_bit_noESC_from3(const int *ix, const int *end, int max,
                                        unsigned int *s) {
  int t1 = huf_tbl_noESC[max - 1];
  unsigned int xlen;
  ...
  {
    unsigned int sum1 = (unsigned int)(sum & 0xffffu);
    unsigned int sum2 = (unsigned int)((sum >> 16u) & 0xffffu);
    unsigned int sum3 = (unsigned int)((sum >> 32u) & 0xffffu);
...
*/
static int count_bit_null(const int *ix, const int *end, int max,
                          unsigned int *s) {
  (void)ix;
  (void)end;
  (void)max;
...
typedef int (*count_fnc)(const int *ix, const int *end, int max,
                         unsigned int *s);

static const count_fnc count_fncs[] = {
    &count_bit_null,        &count_bit_noESC,       &count_bit_noESC_from2,
    &count_bit_noESC_from2, &count_bit_noESC_from3, &count_bit_noESC_from3,
...
static int ix_max_range(const int *ix, const int *end) {
  int max1 = 0, max2 = 0;
...
static int choose_table_nonMMX_with_max(const int *ix, const int *const end,
                                        unsigned int max, int *const _s) {
...
static int choose_table_nonMMX(const int *ix, const int *const end,
                               int *const _s) {
...
inline static void best_huffman_divide_cached(
...
/*	      count_bit							 */
/*************************************************************************/
int noquant_count_bits(lame_internal_flags const *const gfc, gr_info *const gi,
  ...
  if (gi->block_type == SHORT_TYPE) {
    ...
    if (a2 < i)
      gi->table_select[2] = choose_table_nonMMX(ix + a2, ix + i, &bits);
  } else {
    gi->region0_count = 7;
  ...
  if (0 < a1)
    gi->table_select[0] = choose_table_nonMMX(ix, ix + a1, &bits);
  if (a1 < a2)
    gi->table_select[1] = choose_table_nonMMX(ix + a1, ix + a2, &bits);
...
recalc_divide_init(const lame_internal_flags *const gfc,
                   gr_info const *cod_info, int const *const ix,
  ...
  for (r0 = 0; r0 < 16; r0++) {
    ...
    if (a1 >= bigv)
      break;
    r0bits = 0;
    r0t = choose_table_nonMMX_with_max(
        ix, ix + a1, max_cache->range_max[0][r0 + 1], &r0bits);
    ...
    for (r1 = 0; r1 < 8; r1++) {
      ...
      bits = r0bits;
      r1t = choose_table_nonMMX_with_max(
          ix + a1, ix + a2, max_cache->range_max[r0 + 1][r0 + r1 + 2], &bits);
...
inline static void recalc_divide_sub(
    const lame_internal_flags *const gfc, const gr_info *cod_info2,
    gr_info *const gi, const int *const ix,
    const long_block_region_max_cache_t *const max_cache, const int r01_bits[],
  ...
  for (r2 = 2; r2 < SBMAX_l + 1; r2++) {
    ...
    r2t = choose_table_nonMMX_with_max(
        ix + a2, ix + bigv, max_cache->range_max[r2][max_cache->band_count],
        &bits);
    if (gi->part2_3_length <= bits)
      continue;
...
inline static void best_huffman_divide_cached(
  ...
  if (gi->block_type == NORM_TYPE) {
    if (max_cache == 0) {
      long_block_region_max_cache_init(gfc, ix, gi->big_values,
                                       &local_max_cache);
      max_cache = &local_max_cache;
    }
    recalc_divide_init(gfc, gi, ix, max_cache, r01_bits, r01_div, r0_tbl,
                       r1_tbl);
    recalc_divide_sub(gfc, &cod_info2, gi, ix, max_cache, r01_bits, r01_div,
                      r0_tbl, r1_tbl);
  }
  ...
  if (cod_info2.block_type == NORM_TYPE)
    ...
    if (a1 > 0)
      cod_info2.table_select[0] =
          choose_table_nonMMX(ix, ix + a1, (int *)&cod_info2.part2_3_length);
    if (i > a1)
      cod_info2.table_select[1] = choose_table_nonMMX(
 ...
 */
void best_scalefac_store(const lame_internal_flags *gfc, const int gr,
                         const int ch, III_side_info_t *const l3_side) {
  SessionConfig_t const *const cfg = &gfc->cfg;
...
static int mpeg1_scale_bitcount(const lame_internal_flags *gfc,
...
*/
static const int max_range_sfac_tab[6][4] = {{15, 15, 7, 7}, {15, 15, 7, 0},
                                             {7, 3, 0, 0},   {15, 31, 31, 0},
                                             {7, 7, 7, 0},   {3, 3, 0, 0}};
...
static int mpeg2_scale_bitcount(const lame_internal_flags *gfc,
                                gr_info *const cod_info) {
