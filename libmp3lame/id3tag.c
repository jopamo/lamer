...
typedef enum MiscIDs {
  ...
  ID_PCST =
      FRAME_ID('P', 'C', 'S',
               'T') /* iTunes Podcast indicator, only presence important */
  ,
  ID_WFED =
      FRAME_ID('W', 'F', 'E',
...
static int test_tag_spec_flags(lame_internal_flags const *gfc,
                               unsigned int tst) {
...
static int id3v2_add_ucs2_lng(lame_t gfp, uint32_t frame_id,
                              unsigned short const *desc,
                              unsigned short const *text);
static int id3v2_add_latin1_lng(lame_t gfp, uint32_t frame_id, char const *desc,
                                char const *text);

// forward-declare the id3v2_add_utf8_lng function to be used in copyV1ToV2
static int id3v2_add_utf8_lng(lame_t gfp, uint32_t frame_id, char const *desc,
                              char const *text);
...
static void id3v2AddAudioDuration(lame_t gfp, double ms) {
  SessionConfig_t const *const cfg =
      &gfp->internal_flags->cfg; /* caller checked pointers */
  char buffer[1024];
...
void id3tag_genre_list(void (*handler)(int, const char *, void *),
                       void *cookie) {
  if (handler) {
    int i;
 ...
 * */
void id3tag_add_v2_4_UTF8(lame_t gfp) {
  lame_internal_flags *gfc = 0;
 ...
 * */
void id3tag_v2_4_UTF8_only(lame_t gfp) {
  lame_internal_flags *gfc = 0;
...
void id3tag_pad_v2(lame_t gfp) { id3tag_set_pad(gfp, 128); }
...
static unsigned short fromLatin1Char(const unsigned short *s,
                                     unsigned short c) {
  if (s[0] == 0xFFFEu) {
    return swap_bytes(c);
  }
...
static size_t local_ucs2_strdup(unsigned short **dst,
...
static size_t local_ucs2_substr(unsigned short **dst, unsigned short const *src,
...
static unsigned char *writeLoBytes(unsigned char *frame,
                                   unsigned short const *str, size_t n);
...
static int id3tag_set_genre_utf8(lame_t gfp, char const *text) {
...
static FrameDataNode *findNode(id3tag_spec const *tag, uint32_t frame_id,
                               FrameDataNode const *last) {
...
static int isSameDescriptorUcs2(FrameDataNode const *node,
                                unsigned short const *dsc) {
  size_t i;
  if (node->dsc.enc != 1 && node->dsc.dim > 0) {
    return 0;
  }
...
static int id3v2_add_ucs2(lame_t gfp, uint32_t frame_id, char const *lng,
                          unsigned short const *desc,
                          unsigned short const *text) {
  lame_internal_flags *gfc = gfp != 0 ? gfp->internal_flags : 0;
  if (gfc != 0) {
 ...
 */
static int id3v2_add_enc(lame_t gfp, uint32_t frame_id, char const *lng,
                         char const *desc, char const *text, int enc) {
  lame_internal_flags *gfc = gfp != 0 ? gfp->internal_flags : 0;
  if (gfc != 0) {
    FrameDataNode *node = findNode(&gfc->tag_spec, frame_id, 0);
    char lang[4];
...
static int id3v2_add_latin1(lame_t gfp, uint32_t frame_id, char const *lng,
                            char const *desc, char const *text) {
...
static int id3v2_add_utf8(lame_t gfp, uint32_t frame_id, char const *lng,
                          char const *desc, char const *text) {
...
static int id3tag_set_userinfo_latin1(lame_t gfp, uint32_t id,
...
static int id3tag_set_userinfo_utf8(lame_t gfp, uint32_t id,
                                    char const *fieldvalue) {
  char const separator = '=';
  int rc = -7;
  int a = local_char_pos(fieldvalue, separator);
...
static int id3tag_set_userinfo_ucs2(lame_t gfp, uint32_t id,
                                    unsigned short const *fieldvalue) {
...
int id3tag_set_textinfo_utf8(lame_t gfp, char const *id, char const *text) {
  ...
  if (frame_id == ID_WFED) {
    return id3v2_add_utf8_lng(gfp, frame_id, text,
...
int id3tag_set_textinfo_utf16(lame_t gfp, char const *id,
                              unsigned short const *text) {
  uint32_t const frame_id = toID3v2TagId(id);
  if (frame_id == 0) {
    return -1;
...
int id3tag_set_comment_latin1(lame_t gfp, char const *lang, char const *desc,
                              char const *text) {
  if (is_lame_internal_flags_null(gfp)) {
    return 0;
  }
...
int id3tag_set_comment_utf8(lame_t gfp, char const *lang, char const *desc,
                            char const *text) {
  if (is_lame_internal_flags_null(gfp)) {
    return 0;
  }
...
int id3tag_set_comment_utf16(lame_t gfp, char const *lang,
                             unsigned short const *desc,
                             unsigned short const *text) {
  if (is_lame_internal_flags_null(gfp)) {
    return 0;
...
static unsigned char *writeChars(unsigned char *frame, char const *str,
                                 size_t n) {
  while (n--) {
    *frame++ = *str++;
  }
...
static unsigned char *writeUcs2s(unsigned char *frame,
                                 unsigned short const *str, size_t n) {
  if (n > 0) {
...
static unsigned char *set_frame_comment(unsigned char *frame,
                                        FrameDataNode const *node) {
...
static unsigned char *set_frame_custom2(unsigned char *frame,
                                        FrameDataNode const *node) {
...
static unsigned char *set_frame_wxxx(unsigned char *frame,
                                     FrameDataNode const *node) {
...
static unsigned char *set_frame_apic(unsigned char *frame, const char *mimetype,
  ...
  if (mimetype && data && size) {
    ...
    frame =
        set_4_byte_value(frame, (unsigned long)(4 + strlen(mimetype) + size));
    /* clear 2-byte header flags */
    *frame++ = 0;
    *frame++ = 0;
...
int id3tag_set_fieldvalue_utf16(lame_t gfp, const unsigned short *fieldvalue) {
  ...
  if (fieldvalue && *fieldvalue) {
    ...
    if (local_ucs2_strlen(fieldvalue) < (5 + dx) ||
        fieldvalue[4 + dx] != separator) {
      return -1;
    }
    ...
    if (frame_id != 0) {
      ...
      int rc;
      local_ucs2_substr(&txt, fieldvalue, dx + 5,
                        local_ucs2_strlen(fieldvalue));
...
int id3tag_set_fieldvalue_utf8(lame_t gfp, const char *fieldvalue) {
  if (is_lame_internal_flags_null(gfp)) {
    return 0;
  }
  if (fieldvalue && *fieldvalue) {
  ...
  }
  {
    ...
    if (usev2) {
      ...
      if (gfc->tag_spec.albumart && gfc->tag_spec.albumart_size) {
        ...
        if (albumart_mime) {
          tag_size +=
              10 + 4 + strlen(albumart_mime) + gfc->tag_spec.albumart_size;
        }
      }
      {
...
static unsigned char *set_text_field(unsigned char *field, const char *text,
                                     size_t size, int pad) {
  while (size--) {
    if (text && *text) {
      *field++ = *text++;
  ...
  if (test_tag_spec_flags(gfc, CHANGED_FLAG)) {
    ...
    *p++ = 'G';
    ...
    p = set_text_field(p, gfc->tag_spec.comment,
                       gfc->tag_spec.track_id3v1 ? 28 : 30, pad);
