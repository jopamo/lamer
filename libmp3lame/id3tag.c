...
static const int genre_alpha_map[] = {123, 34,  74, 73,  99,  20,  40,  26,  145, 90, 116, 41,  135, 85,  96,  138, 89,  0,   107, 132, 65,  88,  104, 102, 97,  136, 61,  141, 32, 1,
                                      112, 128, 57, 140, 2,   139, 58,  3,   125, 50, 22,  4,   55,  127, 122, 120, 98,  52,  48,  54,  124, 25,  84,  80,  115, 81,  119, 5,   30, 36,
                                      59,  126, 38, 49,  91,  6,   129, 79,  137, 7,  35,  100, 131, 19,  33,  46,  47,  8,   29,  146, 63,  86,  71,  45,  142, 9,   77,  82,  64, 133,
                                      10,  66,  39, 11,  103, 12,  75,  134, 13,  53, 62,  109, 117, 23,  108, 92,  67,  93,  43,  121, 15,  68,  14,  16,  76,  87,  118, 17,  78, 143,
                                      114, 110, 69, 21,  111, 95,  105, 42,  37,  24, 56,  44,  101, 83,  94,  106, 147, 113, 18,  51,  130, 144, 60,  70,  31,  72,  27,  28};

#define GENRE_ALPHA_COUNT ((int)(sizeof genre_alpha_map / sizeof(int)))

#define GENRE_INDEX_OTHER 12

#define FRAME_ID(a, b, c, d) (((unsigned long)(a) << 24) | ((unsigned long)(b) << 16) | ((unsigned long)(c) << 8) | ((unsigned long)(d) << 0))
...
static int id3v2_add_utf8_lng(lame_t gfp, uint32_t frame_id, char const* desc, char const* text);
 ...
 */
static int id3v2_add_enc(lame_t gfp, uint32_t frame_id, char const* lng, char const* desc, char const* text, int enc) {
    lame_internal_flags* gfc = gfp != 0 ? gfp->internal_flags : 0;
    if (gfc != 0) {
        FrameDataNode* node = findNode(&gfc->tag_spec, frame_id, 0);
        char lang[4];
...
static int id3v2_add_utf8(lame_t gfp, uint32_t frame_id, char const* lng, char const* desc, char const* text) {
...
static int id3tag_set_userinfo_utf8(lame_t gfp, uint32_t id, char const* fieldvalue) {
    char const separator = '=';
    int rc = -7;
    int a = local_char_pos(fieldvalue, separator);
    if (a >= 0) {
...
int id3tag_set_textinfo_utf8(lame_t gfp, char const* id, char const* text) {
    ...
    if (isFrameIdMatching(frame_id, FRAME_ID('T', 0, 0, 0)) || isFrameIdMatching(frame_id, FRAME_ID('W', 0, 0, 0))) {
        return id3v2_add_utf8_lng(gfp, frame_id, 0, text);
    }
...
int id3tag_set_comment_utf8(lame_t gfp, char const* lang, char const* desc, char const* text) {
    if (is_lame_internal_flags_null(gfp)) {
        return 0;
    }
...
size_t lame_get_id3v2_tag(lame_t gfp, unsigned char* buffer, size_t size) {
    ...
    }
    {
        ...
        if ((title_length > 30) || (artist_length > 30) || (album_length > 30) || (comment_length > 30) || (gfc->tag_spec.track_id3v1 && (comment_length > 28))) {
            usev2 = 1;
        }
        if (usev2) {
            size_t tag_size;
