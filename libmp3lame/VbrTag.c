...
#define SHIFT_IN_BITS_VALUE(x, n, v) ((x) = (unsigned char)((((unsigned int)(x)) << (n)) | ((unsigned int)(v) & LOW_BITS_MASK(n))))
...
static uint16_t CRC_update_lookup(uint16_t crc, unsigned char const* buffer, int size) {
    while (size >= 8) {
        crc ^= buffer[1] << 8 | buffer[0];

        crc = crc16_lookup[7][crc & 0xFF] ^ crc16_lookup[6][crc >> 8] ^ crc16_lookup[5][buffer[2]] ^ crc16_lookup[4][buffer[3]] ^ crc16_lookup[3][buffer[4]] ^ crc16_lookup[2][buffer[5]] ^
...
static int PutLameVBR(lame_global_flags const* gfp, size_t nMusicLength, uint8_t* pbtStreamBuffer, uint16_t crc) {
    ...
    */
    ...
    uint8_t nLowpass = (((cfg->lowpassfreq / 100.0) + .5) > 255 ? 255 : (cfg->lowpassfreq / 100.0) + .5);
    ...
    if (cfg->findPeakSample)
        nPeakSignalAmplitude = abs((int)((((FLOAT)gfc->ov_rpg.PeakSample) / 32767.0) * pow(2, 23) + .5));
    ...
    nFlags = nAthType + (bExpNPsyTune << 4) + (bSafeJoint << 5) + (bNoGapMore << 6) + (bNoGapPrevious << 7);
    ...
    if (cfg->short_blocks == short_block_forced || cfg->short_blocks == short_block_dispensed || ((cfg->lowpassfreq == -1) && (cfg->highpassfreq == -1)) || /* "-k" */
        (cfg->disable_reservoir && cfg->avg_bitrate < 320) || cfg->noATH || cfg->ATHonly || (nAthType == 0) || cfg->samplerate_in <= 32000)
        bNonOptimal = 1;
