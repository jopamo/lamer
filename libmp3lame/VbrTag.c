...
#define LOW_BITS_MASK(n) ((1u << (n)) - 1u)
#define SHIFT_IN_BITS_VALUE(x, n, v)                                           \
  ((x) = (unsigned char)((((unsigned int)(x)) << (n)) |                        \
                         ((unsigned int)(v) & LOW_BITS_MASK(n))))

static void setLameTagFrameHeader(lame_internal_flags const *gfc,
...
#define MAXFRAMESIZE 2880 /* or 0xB40, the max freeformat 640 32kHz framesize  \
   ...
   */
  {
    int total_frame_size =
        ((cfg->version + 1) * 72000 * kbps_header) / cfg->samplerate_out;
...
static uint16_t CRC_update_lookup(uint16_t crc, unsigned char const *buffer,
                                  int size) {
  while (size >= 8) {
  ...
  while (size--)
    crc = (crc >> 8) ^ crc16_lookup[0][(crc & 0xff) ^ *buffer++];
  return crc;
}
 ...
 *                              id3v2size               : size of id3v2 tag in
 * bytes crc                             : computation of crc-16 of Lame Tag so
 * far (starting at frame sync)
 *
 ****************************************************************************
 */
static int PutLameVBR(lame_global_flags const *gfp, size_t nMusicLength,
  ...
  */
  ...
  uint8_t nLowpass = (((cfg->lowpassfreq / 100.0) + .5) > 255
                          ? 255
                          : (cfg->lowpassfreq / 100.0) + .5);
  ...
  if (cfg->findReplayGain) {
    ...
    nRadioReplayGain |=
        0xC00; /* set originator code to `determined automatically' */
  ...
  if (cfg->findPeakSample)
    nPeakSignalAmplitude = abs(
        (int)((((FLOAT)gfc->ov_rpg.PeakSample) / 32767.0) * pow(2, 23) + .5));
  ...
  nFlags = nAthType + (bExpNPsyTune << 4) + (bSafeJoint << 5) +
           (bNoGapMore << 6) + (bNoGapPrevious << 7);
  ...
  /*Check if the user overrided the default LAME behaviour with some nasty
   * options */

  if (cfg->short_blocks == short_block_forced ||
      cfg->short_blocks == short_block_dispensed ||
      ((cfg->lowpassfreq == -1) && (cfg->highpassfreq == -1)) || /* "-k" */
      (cfg->disable_reservoir && cfg->avg_bitrate < 320) || cfg->noATH ||
      cfg->ATHonly || (nAthType == 0) || cfg->samplerate_in <= 32000)
    bNonOptimal = 1;

  nMisc = nNoiseShaping + (nStereoMode << 2) + (bNonOptimal << 5) +
          (nSourceFreq << 6);
  ...
  pbtStreamBuffer[nBytesWritten] =
      enc_delay >> 4; /* works for win32, does it for unix? */
...
size_t lame_get_lametag_frame(lame_global_flags const *gfp,
