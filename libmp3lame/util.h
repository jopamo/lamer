...
extern ieee754_float32_t fast_log2_table[LOG2_SIZE + 1];
...
static inline ieee754_float32_t fast_log2(ieee754_float32_t x) {
  ...
  /* log2val += fast_log2_table[mantisse]; without interpolation the results are
   * not good */
  log2val += fast_log2_table[mantisse] * (1.0f - partial) +
             fast_log2_table[mantisse + 1] * partial;
...
typedef struct {
  ...
  FLOAT bo_weight[Max(SBMAX_l, SBMAX_s)]; /* band weight long scalefactor bands,
                                             at transition */
...
typedef struct {
  ...
  int bitrate_blocktype_hist[16][4 + 1 +
                                 1]; /*norm/start/short/stop/mixed(short)/sum */
...
typedef struct {
  ...
  int full_outer_loop; /* 0 = stop early after 0 distortion found. 1 = full
                          search */
  ...
  int channels_in;    /* number of channels in the input data stream (PCM or
                         decoded PCM) */
  int channels_out;   /* number of channels in the output data stream (not used
                         for decoding) */
  ...
  float ATH_offset_factor; /* change ATH by this factor, derived from
                              ATH_offset_db */
...
struct lame_internal_flags {
   ...
   * It is very unlikely that a pointer to lame_global_flags has the same 32
   * bits in it's structure (large and other special properties, for instance
   * prime).
   *
   ...
   * test should be "if (flag == 1)" and NOT "if (flag)". Unintended
   * modification of this element will be otherwise misinterpreted as an init.
   */
