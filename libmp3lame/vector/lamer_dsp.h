...
typedef struct lamer_dsp_s {
  ...
  void (*psy_attack_hpf_f32)(FLOAT *dst, FLOAT const *src, int n,
                             FLOAT const *coef);
   ...
   */
  void (*reconstructed_energy_f32)(FLOAT const *xr, int const *ix, FLOAT step,
                                   int n, FLOAT *source_energy,
                                   FLOAT *quant_energy);
  ...
  void (*vbr_quantize_x34)(int *l3, FLOAT const *xr34, unsigned int bw,
                           FLOAT sfpow34);
} lamer_dsp;
