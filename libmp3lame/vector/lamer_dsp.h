 ...
 */
typedef enum lamer_dsp_helper_class_e { LAMER_DSP_CLASS_A_BIT_EXACT = 1, LAMER_DSP_CLASS_B_NUMERICALLY_CLOSE = 2, LAMER_DSP_CLASS_C_DECISION_PATH = 3 } lamer_dsp_helper_class_t;
...
typedef struct lamer_dsp_s {
     ...
     */
    void (*reconstructed_energy_f32)(FLOAT const* xr, int const* ix, FLOAT step, int n, FLOAT* source_energy, FLOAT* quant_energy);
     ...
     */
    FLOAT (*vbr_calc_sfb_noise_x34)(FLOAT const* xr, FLOAT const* xr34, unsigned int bw, uint8_t sf);
    void (*vbr_quantize_x34)(int* l3, FLOAT const* xr34, unsigned int bw, FLOAT sfpow34);
} lamer_dsp;
