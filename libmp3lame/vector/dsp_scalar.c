...
static void scalar_window_mul_f32(FLOAT* dst, FLOAT const* src, FLOAT const* win, int n) {
    int i;
    for (i = 0; i < n; ++i) {
        dst[i] = src[i] * win[i];
    }
...
static FLOAT scalar_vbr_calc_sfb_noise_x34(FLOAT const* xr, FLOAT const* xr34, unsigned int bw, uint8_t sf) {
...
static void scalar_vbr_quantize_x34(int* l3, FLOAT const* xr34, unsigned int bw, FLOAT sfpow34) {
    unsigned int i = bw >> 2u;
    unsigned int const remaining = bw & 0x03u;
...
static void scalar_reconstructed_energy_f32(FLOAT const* xr, int const* ix, FLOAT step, int n, FLOAT* source_energy, FLOAT* quant_energy) {
    FLOAT source = 0.0f;
    FLOAT quant = 0.0f;
    int i;
...
static int simd_env_disabled(char const* env) {
    return env != 0 && (strcmp(env, "0") == 0 || strcmp(env, "off") == 0 || strcmp(env, "false") == 0 || strcmp(env, "disabled") == 0 || strcmp(env, "scalar") == 0);
}

static int simd_env_allows(char const* env, char const* backend) {
    return env == 0 || *env == '\0' || strcmp(env, "auto") == 0 || strcmp(env, backend) == 0;
}
