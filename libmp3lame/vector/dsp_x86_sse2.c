...
static int sse2_experimental_quant_simd_enabled(void) {
    char const* const env = getenv("LAMER_SIMD_EXPERIMENTAL_QUANT");

    return env != 0 && *env != '\0' && strcmp(env, "0") != 0 && strcmp(env, "off") != 0 && strcmp(env, "false") != 0 && strcmp(env, "disabled") != 0;
}

static void sse2_abs_f32(FLOAT* dst, FLOAT const* src, int n) {
...
static FLOAT sse2_abs_max_f32(FLOAT const* src, int n, FLOAT floor) {
    __m128 const abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
...
static FLOAT sse2_max_f32(FLOAT const* src, int n) {
...
static FLOAT sse2_sum_sq_f32(FLOAT const* src, int n) {
    __m128 const abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
    FLOAT tmp[4];
    FLOAT sum = 0.0f;
    int i = 0;
...
static FLOAT sse2_dot_f32(FLOAT const* a, FLOAT const* b, int n) {
    FLOAT tmp[4];
    FLOAT sum = 0.0f;
    int i = 0;
...
static void sse2_window_mul_f32(FLOAT* dst, FLOAT const* src, FLOAT const* win, int n) {
    int i = 0;
...
static void sse2_vbr_k_34_4(__m128 scaled, int idx[4]) {
    FLOAT adj_tmp[4];
...
static FLOAT sse2_vbr_calc_sfb_noise_x34(FLOAT const* xr, FLOAT const* xr34, unsigned int bw, uint8_t sf) {
    __m128 const abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
    ...
    while (i-- > 0) {
        ...
        xfsf += (diff[0] * diff[0] + diff[1] * diff[1]) + (diff[2] * diff[2] + diff[3] * diff[3]);
...
static void sse2_vbr_quantize_x34(int* l3, FLOAT const* xr34, unsigned int bw, FLOAT sfpow34) {
...
static void sse2_reconstructed_energy_f32(FLOAT const* xr, int const* ix, FLOAT step, int n, FLOAT* source_energy, FLOAT* quant_energy) {
    __m128 const abs_mask = _mm_castsi128_ps(_mm_set1_epi32(0x7fffffff));
