...
static FLOAT ATHformula_GB(FLOAT f, FLOAT value, FLOAT f_min, FLOAT f_max) {
    ...
    ath = 3.640 * pow(f, -0.8) - 6.800 * exp(-0.6 * pow(f - 3.4, 2.0)) + 6.000 * exp(-0.15 * pow(f - 8.7, 2.0)) + (0.6 + 0.04 * value) * 0.001 * pow(f, 4.0);
    return ath;
}
...
int FindNearestBitrate(int bRate, /* legal rates from 8 to 320 */
                       int version,
                       int samplerate) { /* MPEG-1 or MPEG-2 LSF */
    int bitrate;
    int i;
...
int nearestBitrateFullIndex(uint16_t bitrate) {
    ...
    const int full_bitrate_table[] = {8, 16, 24, 32, 40, 48, 56, 64, 80, 96, 112, 128, 160, 192, 224, 256, 320};
...
static int fill_buffer_resample(lame_internal_flags* gfc, sample_t* outbuf, int desired_len, sample_t const* inbuf, int len, int* num_used, int ch) {
...
void fill_buffer(lame_internal_flags* gfc, sample_t* const mfbuf[2], sample_t const* const in_buffer[2], int nsamples, int* n_in, int* n_out) {
    SessionConfig_t const* const cfg = &gfc->cfg;
    ...
    if (isResamplingNecessary(cfg)) {
        do {
            nout = fill_buffer_resample(gfc, &mfbuf[ch][mf_size], framesize, in_buffer[ch], nsamples, n_in, ch);
        } while (++ch < nch);
        *n_out = nout;
    }
    else {
...
void init_log_table(void) {
    ...
    if (!init) {
        for (j = 0; j < LOG2_SIZE + 1; j++)
            fast_log2_table[j] = log(1.0f + j / (ieee754_float32_t)LOG2_SIZE) / log(2.0f);
    }
    init = 1;
}
