...
void calloc_aligned(aligned_pointer_t *ptr, unsigned int size,
                    unsigned int bytes) {
  if (ptr) {
    if (!ptr->pointer) {
...
ATHformula(SessionConfig_t const *cfg, FLOAT freq) {
  ...
  case 0:
    ...
    break;
  case 1:
    ath = ATHformula_GB(freq, -1, 0.1f,
                        24.0f); /*over sensitive, should probably be removed */
    break;
  case 2:
    ath = ATHformula_GB(freq, 0, 0.1f, 24.0f);
    break;
  case 3:
    ath = ATHformula_GB(freq, 1, 0.1f, 24.0f) +
          6; /*modification of GB formula by Roel */
    break;
  case 4:
...
int nearestBitrateFullIndex(uint16_t bitrate) {
  ...
  const int full_bitrate_table[] = {8,  16,  24,  32,  40,  48,  56,  64, 80,
                                    96, 112, 128, 160, 192, 224, 256, 320};

  int lower_range = 0, lower_range_kbps = 0, upper_range = 0,
      upper_range_kbps = 0;
   ...
   * if loop ends without breaking then we were correct above that the value was
   * 320
   */
  for (b = 0; b < 16; b++) {
...
static int fill_buffer_resample(lame_internal_flags *gfc, sample_t *outbuf,
                                int desired_len, sample_t const *inbuf, int len,
                                int *num_used, int ch) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  EncStateVar_t *esv = &gfc->sv_enc;
  double resample_ratio =
      (double)cfg->samplerate_in / (double)cfg->samplerate_out;
  int BLACKSIZE;
  FLOAT offset;
  int i, j = 0, k;
  int filter_l;
...
void fill_buffer(lame_internal_flags *gfc, sample_t *const mfbuf[2],
                 sample_t const *const in_buffer[2], int nsamples, int *n_in,
                 int *n_out) {
  SessionConfig_t const *const cfg = &gfc->cfg;
  ...
  if (isResamplingNecessary(cfg)) {
    do {
      nout = fill_buffer_resample(gfc, &mfbuf[ch][mf_size], framesize,
    ...
    *n_out = nout;
    ...
    do {
      memcpy(&mfbuf[ch][mf_size], &in_buffer[ch][0],
             nout * sizeof(mfbuf[0][0]));
    } while (++ch < nch);
    *n_out = nout;
    *n_in = nout;
  }
 ...
 * maximum *relative* error can be high when x is almost 1 because
 * error/log10(x) tends toward x/e
 *
 * use it if typical RESULT values are > 1e-5 (for example if x>1.00001 or
 * x<0.99999) or if the relative precision in the domain around 1 is not
 * important (result in 1 is exact and 0)
 *
 ***********************************************************************/

ieee754_float32_t fast_log2_table[LOG2_SIZE + 1];
...
void init_log_table(void) {
  ...
  if (!init) {
    for (j = 0; j < LOG2_SIZE + 1; j++)
      fast_log2_table[j] =
          log(1.0f + j / (ieee754_float32_t)LOG2_SIZE) / log(2.0f);
  }
  init = 1;
}
