...
typedef enum vbr_mode_e {
  ...
  vbr_default =
      vbr_mtrh /* change this to change the default VBR mode of LAME */
} vbr_mode;
...
/*
  ...
  algorithms. quality=0..9.  0=best (very slow).  9=worst. recommended:  2
  near-best quality, not too slow 5     good quality, fast 7     ok quality,
  really fast
*/
...
int CDECL
lame_set_experimental_short_transient_redistribute(lame_global_flags *, int);
int CDECL
lame_get_experimental_short_transient_redistribute(const lame_global_flags *);

/* narrow short-block transient redistribution candidate */
int CDECL lame_set_safe_short_transient_redistribute(lame_global_flags *, int);
int CDECL lame_get_safe_short_transient_redistribute(const lame_global_flags *);
...
int CDECL lame_encode_buffer_ieee_double(
    lame_t gfp, const double pcm_l[], /* PCM data for left channel     */
    const double pcm_r[],             /* PCM data for right channel    */
    const int nsamples, unsigned char *mp3buf, const int mp3buf_size);
int CDECL lame_encode_buffer_interleaved_ieee_double(
    lame_t gfp, const double pcm[], /* PCM data for left and right
                                       channel, interleaved          */
    const int nsamples, unsigned char *mp3buf, const int mp3buf_size);
...
int lame_encode_buffer_interleaved_int(
    lame_t gfp, const int pcm[], /* PCM data for left and right
                                    channel, interleaved          */
    const int nsamples,          /* number of samples per channel,
                                    _not_ number of samples in
                                    pcm[]                         */
 ...
 */
int CDECL
lame_init_bitstream(lame_global_flags *gfp); /* global context handle */
...
void CDECL lame_bitrate_kbps(const lame_global_flags *gfp,
                             int bitrate_kbps[14]);
void CDECL lame_stereo_mode_hist(const lame_global_flags *gfp,
                                 int stereo_mode_count[4]);

void CDECL lame_bitrate_stereo_mode_hist(const lame_global_flags *gfp,
                                         int bitrate_stmode_count[14][4]);

void CDECL lame_block_type_hist(const lame_global_flags *gfp,
                                int btype_count[6]);
...
void CDECL id3tag_genre_list(void (*handler)(int, const char *, void *),
                             void *cookie);
...
/* experimental */
int CDECL id3tag_set_textinfo_latin1(lame_t gfp, char const *id,
                                     char const *text);

/* experimental */
int CDECL id3tag_set_comment_latin1(lame_t gfp, char const *lang,
                                    char const *desc, char const *text);

/* experimental */
int CDECL id3tag_set_fieldvalue_utf16(lame_t gfp,
                                      const unsigned short *fieldvalue);
...
/* experimental */
int CDECL id3tag_set_textinfo_utf8(lame_t gfp, char const *id,
                                   char const *text);

/* experimental */
int CDECL id3tag_set_comment_utf8(lame_t gfp, char const *lang,
                                  char const *desc, char const *text);
